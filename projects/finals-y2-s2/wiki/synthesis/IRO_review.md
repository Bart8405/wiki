# IRO Highlights & Review

_Concepts and questions automatically extracted from your notes._

**Last updated:** 2026-05-20

## Table of Contents

- [[#Week 1 — Old Debates in a New World]]
- [[#Week 2 — Power, Institutions, and Ideas]]
- [[#Week 3 — Security Dilemmas and Nuclear Proliferation]]
- [[#Week 4 — Non-Western IR Theory (Denney)]]
- [[#Week 6 — Global Migration: Mobility and Security]]
- [[#Week 7 — War, Peace, and International Security (Wolff)]]
- [[#Week 8 — Power, Liberalism, and World Order (Wolff)]]
- [[#Week 9 — Global Governance and EU Regional Integration]]
- [[#Week 10 — Nation, Race, and Gender]]
- [[#Week 11 — Globalization and Populism (Denney)]]
- [[#Week 12 — Anthropocene and Global Futures (Denney)]]

**One-page revision sheets (quick-access cheat sheets):**
- [[#Week 1 — Old Debates in a New World]] (revision)
- [[#Week 2 — Power, Institutions, and Ideas|Week 2]] (revision)
- [[#Week 3 — Security Dilemmas and Nuclear Proliferation|Week 3]] (revision)
- [[#Week 4 — Non-Western IR Theory|Week 4]] (revision)
- [[#Week 5 — Critical and Decentring Perspectives|Week 5]] (revision)
- [[#Week 6 — Global Migration: Mobility and Security|Week 6]] (revision)
- [[#Week 7 — War, Peace and International Security|Week 7]] (revision)
- [[#Week 8 — Power, Liberalism and World Order|Week 8]] (revision)
- [[#Week 9 — Global Governance and EU Regional Integration|Week 9]] (revision)
- [[#Week 10 — Nation, Race and Gender|Week 10]] (revision)
- [[#Week 11 — Globalization and Populism|Week 11]] (revision)
- [[#Week 12 — Anthropocene and Global Futures|Week 12]] (revision)

---

## All Terms

| Term | Source | One-liner |
|---|---|---|
| Acharya, Amitav (key thinker) | Week 4 / Week 8 / Week 12 | Singaporean-American IR scholar; multiplex world order, deprovincialization, Global IR |
| Clash of Civilizations (Huntington 1993) | Week 1 — Old Debates | Post-Cold War conflicts will be cultural/civilizational, not ideological |
| Civic nationalism | Week 10 — Nation/Race/Gender | Membership via political institutions — Ukraine case |
| Constructivism | Week 2 — IR Theories | State interests constructed through interaction — "anarchy is what states make of it" |
| End of History (Fukuyama 1989) | Week 1 — Old Debates | Ideological evolution terminates at liberal democracy — Fukuyama's thesis |
| End of New START (Feb 2026) | Week 3 — Nuclear | First time in 50+ years with no binding US-Russian nuclear limits |
| Ethnic nationalism | Week 10 — Nation/Race/Gender | Membership via descent and language — South Korea case |
| Imagined community (Anderson) | Week 10 — Nation/Race/Gender | Nation as horizontal community made thinkable by print capitalism |
| Liberal International Order (LIO) | Week 2 — IR Theories | Post-1945 US-led system: open markets + institutions + democratic legitimacy |
| Liberal intergovernmentalism (EU sense, Moravcsik) | Week 9 — EU/Governance | Member-state bargains rooted in domestic economic interests — 3-step model |
| Masculinist nationalism (Nagel) | Week 10 — Nation/Race/Gender | Nation as project of gender-building — honour, sacrifice, defence |
| Neorealism (structural realism) | Week 2 — IR Theories | State behaviour driven by anarchic structure, not human nature (Waltz) |
| Normative power Europe (Manners) | Week 9 — EU/Governance | EU shapes "normal" via norms and standards, not military force |
| Nuclear taboo | Week 3 — Nuclear | Normative inhibition against using nukes — held since 1945 |
| Offensive realism | Week 2 — IR Theories | Great powers maximise relative power; seek regional hegemony (Mearsheimer) |
| Return of History (Kagan 2007) | Week 1 — Old Debates | Great-power competition never went away — the realist counter |
| Sagan's three models (1996) | Week 3 — Nuclear | Security + domestic politics + norms — multicausality at the heart of proliferation |
| Security dilemma | Week 3 — Nuclear | Defensive measures perceived as threats → arms-race spiral |
| Subsidiarity (EU sense) | Week 9 — EU/Governance | EU acts only where more effective than national level — NOT Acharya's sense |
| Waltz-Sagan debate | Week 3 — Nuclear | "More may be better" (Waltz) vs "more will be worse" (Sagan) |
| Acharya's critique of Friedberg & Kang | Week 4 — NWIRT | Both apply Western frames to Asia — balancing/bandwagoning is too narrow |
| Anthropocene | Week 12 — Anthropocene | Geological epoch dominated by human activity; planetary processes exceed state scale |
| ASEAN regionalism (consensus-based) | Week 4 — NWIRT | Sovereignty-respecting alternative to the EU model |
| Asian exceptionalism / "Asian values" / Tianxia | Week 4 — NWIRT | Replacing Western parochialism with reverse parochialism — Acharya warns against this |
| asylum shopping | Week 6 — Migration | Applying for asylum in multiple countries; Dublin was meant to stop it |
| Audra Mitchell (key thinker) | Week 12 — Anthropocene | "Is IR going extinct?" — ontological critique of IR's anthropocentrism |
| balance of threat (Walt) | Week 4 — NWIRT | States balance against *threats*, not raw power — refutes Kang's capability argument |
| bandwagoning | Week 4 — NWIRT | Aligning with the stronger power to share its gains — Kang says Asia does this |
| Barkawi, Tarak (key thinker) | Week 7 — War | Decolonising war — the Eurocentric concept must be rethought |
| Beijing Consensus | Week 8 — World Order | Development without liberal democracy — China's alternative model |
| Belt and Road Initiative (BRI) | Week 8 — World Order | Xi's flagship bilateral infrastructure programme; criticised as "debt trap diplomacy" |
| BRICS | Week 8 — World Order | Brazil/Russia/India/China/South Africa — alternative to G7/G8 |
| Clausewitz on war | Week 7 — War | "War is a continuation of political intercourse, with the addition of other means" |
| comparative regionalism | Week 8 — World Order | Study of regional integration beyond Europe (ASEAN, AU…) |
| constitutive localization | Week 4 — NWIRT | Foreign ideas blend two-way into local norms (ASEAN Way) |
| Dani Rodrik (key thinker) | Week 8 / Week 11 | Globalization trilemma + demand/supply of populism |
| decolonising war | Week 7 — War | Barkawi: the Eurocentric concept of war excludes colonial conflicts |
| democratic peace theory | Week 7 — War | Liberal democracies almost never fight each other |
| deprovincialization | Week 4 — NWIRT | Acharya's path 3 — localise Western IRT to Asia + universalise Asian constructs |
| "Dragon" states (Kilcullen) | Week 7 — War | Russia, China — refused to orbit US order; use "three warfares" (psychological, public opinion, lawfare) |
| double-binding | Week 4 — NWIRT | Enmeshing *both* China and the US in regional institutions (ASEAN's strategy) |
| Friedberg's "ripe for rivalry" | Week 4 — NWIRT | Neorealism applied to Asia — pessimistic predictions of conflict |
| functionalism + neofunctionalism + spillover | Week 8 — World Order | Integration theories: states cooperate functionally → spillover drags in politics (Mitrany → Haas) |
| Global IR | Week 4 — NWIRT | Acharya's preferred label — broadening, not replacing, Western IR |
| globalization trilemma (Rodrik) | Week 8 / Week 11 | Deep globalization + nation-state + democracy — pick any two |
| globalization of rage (Mishra) | Week 11 — Populism | Globalization produced psychic dislocation; modernity promised dignity and didn't deliver |
| hegemonic acceptance (Gramscian) | Week 4 — NWIRT | Non-Western scholars internalise the assumption that Western theory is "better" |
| hegemonic stability theory | Week 4 / Week 8 | A single dominant power stabilises the international system (Ikenberry restates) |
| Hobson's choice | Week 6 — Migration | Apparent choice with only one real option |
| human security (Mahbub ul Haq) | Week 4 — NWIRT | Pakistani-origin concept, adopted globally via UNDP — example of subsidiarity |
| institutional power | Week 8 — World Order | Power embedded in the rules of the game — US shaped UN/WTO/IMF |
| intergovernmentalism + liberal intergovernmentalism | Week 8 — World Order | States are the key actors in integration (Hoffmann → Moravcsik) |
| internal market spillover into internal security | Week 6 — Migration | How removing internal EU borders created EU security integration |
| Just War Theory (jus ad bellum / in bello / post bellum) | Week 7 — War | Middle ground between realism (war always legitimate) and pacifism (never) |
| Kang's "Asian hierarchy" thesis | Week 4 — NWIRT | Culturalist realism — Asian states bandwagon; rising China restores stability |
| liberal paradox | Week 6 — Migration | States trapped between economic openness and political closure (Hollifield) |
| Mandala system | Week 4 — NWIRT | Pre-modern SE Asian order — overlapping circles, fuzzy borders, personal hegemony |
| migration state | Week 6 — Migration | Third state function alongside garrison and trading (Hollifield) |
| multiplex world order | Week 8 — World Order | Acharya — diverse actors/norms; "multiple shows on offer" — not just multipolarity |
| neoconservatism (Kagan post-9/11) | Week 8 — World Order | US asserted unilateral primacy; "America did not change on 9/11 — it became more itself" |
| new wars (Kaldor) | Week 7 — War | Post-Cold War conflicts blurring war/crime, non-state actors, identity-driven |
| nonintervention / nonalignment | Week 4 — NWIRT | Bandung-era norms; foundational global norms via Asian/African states |
| NWIRT (Non-Western IR Theory) | Week 4 — NWIRT | Argues mainstream IR is too rooted in Western history to capture non-Western experience |
| Orientalism (Said) | Week 4 — NWIRT | West's systematic construction of "the East" as exotic/inferior Other |
| post-humanism | Week 12 — Anthropocene | Decenters the human as sole subject of ethics; nonhumans as actors |
| populism: demand vs. supply (Rodrik) | Week 11 — Populism | Demand from globalization shocks; supply (left/right form) depends on cleavage activated |
| populism as repertoire vs nationalism as structure | Week 11 — Populism | Populism is a style you pick up; nationalism is a structural condition |
| power as resource vs. outcome | Week 8 — World Order | What you possess (realist) vs. what you can achieve relationally (constructivist) |
| precariat (Standing) | Week 8 / Week 11 | Workers on insecure, low-wage, fixed-term labor; sizable bloc voting differently |
| Private Military Security Industry (PMSI) | Week 7 — War | Commercial firms providing armed security; 1 in 4 US personnel in Iraq/Afghanistan |
| R2P (Responsibility to Protect) | Week 7 — War | 2005 UN doctrine — sovereignty is conditional on protecting population |
| ReArm Europe / SAFE instrument | Week 7 — War | EU's €800B 2025 defence plan; new lending instrument in response to Ukraine |
| realisms (subaltern / neoclassical / defensive) | Week 4 — NWIRT | New realisms that make the school more relevant to non-Western and Asian cases |
| security communities (Deutsch) | Week 7 — War | Groups of states where war is "unthinkable" — EU/NATO area |
| security continuum | Week 6 — Migration | Institutional link between migration, crime, and terrorism (Bigo/Huysmans) |
| smart power (Nye) | Week 8 — World Order | Combining hard and soft power strategically |
| subsidiarity (Acharya sense) | Week 4 — NWIRT | Local norms diffuse outward to global governance — *not* the EU meaning |
| Tianxia ("all under heaven") | Week 4 — NWIRT | Chinese philosophical concept; Acharya warns it romanticises hierarchy |
| tributary system | Week 4 — NWIRT | Pre-modern Sinic order — hierarchy, not anarchy; not consistently peaceful |
| twisted postcoloniality (S. Korea) | Week 4 — NWIRT | Colonised by Japan, then US-dependent — Korean scholars trapped between mimicry and marginality |
| world order | Week 8 — World Order | "Distribution of power giving rise to stable patterns of relationships" (Heywood) |
| ZOPFAN | Week 4 — NWIRT | Zone of Peace, Freedom and Neutrality — SE Asian initiative to keep the region free from great-power competition |

---

## Week 1 — Old Debates in a New World

| Term | One-liner |
|---|---|
| End of History (Fukuyama) | Liberal democracy as the endpoint of ideological evolution |
| Clash of Civilizations (Huntington) | Post-Cold War conflicts will be cultural/civilizational |
| Return of History (Kagan) | Great-power competition never went away — the realist counter |

---

### The three post-Cold War visions (Fukuyama + Huntington + Kagan)
*(The course's interpretive grid — all three tested against 2014–2026 reality)*

**End of History (Fukuyama 1989):** The thesis that with the Cold War's end, ideological evolution has terminated at liberal democracy + market economy. Not "the end of events" but the end of the *contest of ideas about how to organise society*. Rooted in Hegel's idea of history as directional. Fukuyama's confidence in liberalism's inevitability has since been substantially revised by the author himself.

**Clash of Civilizations (Huntington 1993):** Post-Cold War conflicts will be *cultural/civilizational*, not ideological or economic. Huntington identified 7–8 civilizations (Western, Confucian, Japanese, Islamic, Hindu, Slavic-Orthodox, Latin American, 'possibly' African). Most violent conflicts will occur along "civilizational fault lines" — especially West vs Islam and West vs China.

**Three lecture critiques of Huntington:** (1) Civilizations don't inevitably clash — India–US security ties contradict it. (2) Too many intra-civilizational conflicts (Sunni–Shia, Russia–Ukraine). (3) Essentialises cultures as monolithic.

**Return of History (Kagan 2007):** The world has *not* been transformed by 1989. Traditional power struggles — driven by ambition and competition — still shape global politics. The ideological struggle between liberalism and autocracy has *re-emerged*. Kagan's key tension: tensions between democracies (US–EU) and between autocracies (China–Russia) may be as significant as the cross-divide.

**How they fit together:** Fukuyama = liberal optimist (convergence); Huntington = cultural pessimist (fragmentation); Kagan = realist buzz-killer (great-power competition returned). Acharya's *multiplex world* (Week 8) is the contemporary counter — neither a single liberal endpoint, nor a civilizational clash, nor simple great-power rivalry, but genuine diversity. Empirically, Kagan's frame fits 2014–2026 best; Fukuyama fits worst (V-Dem 2025: autocracies > democracies; liberal democracies the least common regime type).

**Explained easily:** *Fukuyama:* "The big debate about how society should be organised is over — liberal democracy won." *Huntington:* "Forget ideology — culture drives conflict now; the West will fight Islam and China." *Kagan:* "Nothing fundamental changed in 1989; power and geopolitical competition are back." All three are tested against Russia's 2022 invasion of Ukraine and China's challenge to US primacy.

**Connected to:** → [[#multiplex world order + multipolarity distinction|multiplex world order]] (Week 8) — Acharya's critique of all three; → [[#democratic peace theory|democratic peace theory]] (Week 7) — descendant of Fukuyama's liberal peace thesis; → [[#Anthropocene + post-humanism|Anthropocene]] (Week 12) — Denney closes by revisiting all three frames with planetary stakes added.

**Seen in:**
- [Week 1 - Old Debates in a New World.md:L21](projects/finals-y2-s2/IRO/Summaries/Week%201%20-%20Old%20Debates%20in%20a%20New%20World.md) — *"==**End of History (Fukuyama 1989)**=="*
- [Week 1 - Old Debates in a New World.md:L23](projects/finals-y2-s2/IRO/Summaries/Week%201%20-%20Old%20Debates%20in%20a%20New%20World.md) — *"==**Clash of Civilizations (Huntington 1993)**=="*
- [Week 1 - Old Debates in a New World.md:L25](projects/finals-y2-s2/IRO/Summaries/Week%201%20-%20Old%20Debates%20in%20a%20New%20World.md) — *"==**Return of History (Kagan 2007)**=="*

---

## Week 2 — Power, Institutions, and Ideas

| Term | One-liner |
|---|---|
| Neorealism + Offensive realism | State behaviour driven by anarchic structure (Waltz) / great powers maximise power (Mearsheimer) |
| Constructivism + "where do those interests come from?" | Interests are socially constructed — Wendt's challenge to realism and liberalism |
| Liberal International Order (LIO) | Post-1945 US-led system; Mearsheimer says it was "bound to fail" |

---

### Neorealism + Offensive realism
*(Waltz + Mearsheimer — the realist backbone of the course)*

**Neorealism (Waltz 1979):** State behaviour is shaped by the *structure* of the international system — anarchy plus distribution of capabilities — not human nature (classical realism). All states face the same structural pressure: survive under anarchy. The dominant realist tradition since 1979.

**Offensive realism (Mearsheimer):** Anarchy + uncertainty about intentions incentivise states to *maximise* relative power. Great powers aim for regional hegemony (the highest feasible goal). Non-great powers face strong constraints and often bandwagon or pursue opportunistic gains. Explains China's rise as a challenge to US primacy in East Asia — and forms the theoretical basis of Mearsheimer's "Bound to Fail" critique of the LIO (Week 2/Week 8).

**Why they differ from each other and from defensive realism:** Neorealism says the structure constrains; offensive realism says the constraint pushes toward *power maximisation*. Defensive realism (Waltz, Walt) says states prefer *security*, not hegemony — restraint and balancing are the rational responses.

**Connected to:** → [[#bandwagoning + balance of threat|bandwagoning + balance of threat]] (Week 4) — offensive realism predicts China's rise triggers balancing; Kang's "Asian hierarchy" thesis argues bandwagoning instead; → [[#security dilemma|security dilemma]] (Week 3) — the mechanism that makes offensive realism's logic grip; → [[#multiplex world order + multipolarity distinction|neoconservatism + hegemonic stability]] (Week 8) — "Bound to Fail" is Mearsheimer's offensive-realist obituary for the LIO.

**Seen in:**
- [Week 2 - Power Institutions and Ideas.md:L23](projects/finals-y2-s2/IRO/Summaries/Week%202%20-%20Power%20Institutions%20and%20Ideas.md) — *"==**Neorealism (structural realism)**=="*
- [Week 2 - Power Institutions and Ideas.md:L24](projects/finals-y2-s2/IRO/Summaries/Week%202%20-%20Power%20Institutions%20and%20Ideas.md) — *"==**Offensive realism**=="*

---

### Constructivism + "where do those interests come from?"
*(Wendt — the third tradition, and its central challenge to the other two)*

**Constructivism:** State behaviour is shaped by shared ideas, identities, and norms — not just material power. State interests are not given; they are *constructed* through interaction. Anarchy is not fixed: its meaning depends on how states interpret their relationships. (Wendt 1992, Onuf, Finnemore, Sikkink.)

**"Where do those interests come from?"** The constructivist challenge to both realism and liberalism: both traditions treat state interests as *given* (realism: power/security; liberalism: rational cooperation). Constructivism asks: how were those interests constructed? What made Russia *interested* in being an empire? What made Germany *uninterested* in acquiring nukes? The answer always involves history, identity, and norms — not just power distribution.

**Wendt's three cultures of anarchy:** The same anarchy can produce three different "cultures" depending on shared identity and practice:
- *Hobbesian:* states as existential threats, kill-or-be-killed (early 20th c. Europe).
- *Lockean:* rivals who recognise sovereignty but compete (Cold War).
- *Kantian:* partners with shared norms, war "unthinkable" (EU/NATO).

**Explained easily:** A high-school cafeteria under the same rules can have Mean Girls (Hobbesian), sports rivalry (Lockean), or study-group solidarity (Kantian). Same anarchy; different politics. The identity of the actors — not the structure alone — determines which culture prevails.

**Connected to:** → [[#NWIRT + Global IR|NWIRT + Global IR]] (Week 4) — constructivism is the epistemological foundation for Acharya's claim that identity and norms shape Asian IR; → [[#hegemonic acceptance (Gramscian)|hegemonic acceptance (Gramscian)]] — Gramsci's hegemony-by-consent is constructivism's closest IPE cousin; → [[#Anthropocene + post-humanism|Anthropocene]] (Week 12) — Mitchell's post-humanism extends constructivism's "question the assumptions" move to the discipline itself.

**Seen in:**
- [Week 2 - Power Institutions and Ideas.md:L32](projects/finals-y2-s2/IRO/Summaries/Week%202%20-%20Power%20Institutions%20and%20Ideas.md) — *"==**Constructivism**=="*
- [Week 2 - Power Institutions and Ideas.md:L122](projects/finals-y2-s2/IRO/Summaries/Week%202%20-%20Power%20Institutions%20and%20Ideas.md) — *"Constructivists ask: ==**where do those interests come from?**=="*

---

### Liberal International Order (LIO)
*(Ikenberry + Mearsheimer's critique — the post-1945 order under strain)*

**Definition:** The post-1945 US-led international system combining three liberal strands: (1) open markets — interdependence liberalism (economic exchange raises conflict cost); (2) rules and institutions — neoliberal institutionalism (reduces uncertainty); (3) democratic legitimacy — republican liberalism (democracies are more rule-bound). The order depends on *legitimacy + restraint*, not force alone. (Ikenberry, Deudney, Lissner.)

**Mearsheimer's "Bound to Fail" (2019):** The LIO required *liberal hegemony* — US dominance to enforce liberal norms. This provoked nationalist backlash, great-power resistance (China, Russia), and costly interventions (Iraq 2003, Libya 2011). The order was a product of *unipolarity* and cannot survive the return of great-power competition. This is the realist critique of liberalism.

**Liberal and constructivist responses:** Liberals argue institutions are *resilient* — they have adapted before and the architecture is sticky. Constructivists argue "failure" is a *narrative choice* — norms of sovereignty, human rights, and multilateralism are deeply embedded; even violators justify themselves in those terms.

**Connected to:** → [[#multiplex world order + multipolarity distinction|multiplex world order + neoconservatism]] (Week 8) — the LIO's fraying creates the multiplex opening; → [[#institutional power|institutional power]] (Week 8) — the mechanism by which the US retains influence as material dominance peaks; → [[#BRI + BRICS + Beijing Consensus|BRI + BRICS]] (Week 8) — China's bid to build parallel institutional architecture outside the LIO.

**Seen in:**
- [Week 2 - Power Institutions and Ideas.md:L31](projects/finals-y2-s2/IRO/Summaries/Week%202%20-%20Power%20Institutions%20and%20Ideas.md) — *"==**Liberal International Order (LIO)**=="*
- [Week 2 - Power Institutions and Ideas.md:L154](projects/finals-y2-s2/IRO/Summaries/Week%202%20-%20Power%20Institutions%20and%20Ideas.md) — *"Mearsheimer says the LIO required ==**liberal hegemony**=="*

---

## Week 3 — Security Dilemmas and Nuclear Proliferation

| Term | One-liner |
|---|---|
| Security dilemma | Defensive measures perceived as threats → spiral, even without hostile intent |
| Nuclear taboo | Normative inhibition against using nukes — held since 1945 (constructivist exhibit A) |
| Sagan's three models + Waltz-Sagan debate | Security · domestic politics · norms — multicausality; "more may be better" vs "more will be worse" |
| End of New START (Feb 5, 2026) | First time in 50+ years with no binding US-Russian nuclear arsenal limits |

---

### Security dilemma
*(The foundational mechanism of Week 3 — and a realism cornerstone)*

**Definition:** When one state's efforts to increase its own security make other states *less* secure, triggering a spiral of competition. Defensive measures get perceived as offensive threats. Uncertainty about intentions in an anarchic system amplifies fear. Arms races can emerge even when no state desires conflict.

**Applied to nuclear proliferation:** "Proliferation begets proliferation" (Sagan 1996, p.57). One state's bomb triggers neighbours to seek their own — a chain reaction. The US-Soviet race from a handful of warheads in 1945 to ~70,000 at the Cold War peak is the canonical illustration.

**Applied to Russia-NATO:** NATO expansion eastward was perceived by Russia as an intolerable security threat, triggering counter-responses — whether or not you agree with that framing, the security-dilemma *mechanism* is what realists cite to explain the lead-up to 2022.

**Connected to:** → [[#Neorealism + Offensive realism|neorealism + offensive realism]] (Week 2) — the security dilemma is what makes structural anarchy so dangerous in Mearsheimer's world; → [[#Sagan's three models + Waltz-Sagan debate|Sagan's security model]] — the first of his three models; it explains why states feel *compelled* to build nukes even when they'd prefer not to.

**Seen in:**
- [Week 3 - Nuclear Proliferation.md:L19](projects/finals-y2-s2/IRO/Summaries/Week%203%20-%20Nuclear%20Proliferation.md) — *"==**Security dilemma**=="*

---

### Nuclear taboo

**Definition:** The normative inhibition against *using* nuclear weapons. Has held since 1945 — Hiroshima and Nagasaki remain the only combat uses. The taboo is not enforced by formal treaty or material incapacity; nuclear states *could* use them in limited conflicts. The restraint is *normative*. Tannenwald (1999, 2007) is the canonical theorist.

**Why it matters:** The nuclear taboo is constructivism's Exhibit A in IR. It demonstrates that norms can constrain behaviour independently of material power — no treaty compels nuclear restraint, yet the taboo has held across 80 years and many crises. If state behaviour were solely determined by power and material incentives (realist view), a state facing defeat might rationally use nuclear weapons. The fact that none have is a constructivist data point.

**The taboo under pressure:** Russia's rhetorical nuclear threats during the Ukraine war (2022–) test the taboo by *signalling* willingness to cross it without actually crossing it. The taboo creates restraint; threatening to violate it creates coercive leverage.

**Connected to:** → [[#Sagan's three models + Waltz-Sagan debate|Sagan's norms model]] — the nuclear taboo is the clearest example of his third model operating as a *disincentive* to use; → [[#Constructivism + "where do those interests come from?"|constructivism]] (Week 2) — the taboo shows how norms constrain even the most material of security choices.

**Seen in:**
- [Week 3 - Nuclear Proliferation.md:L27](projects/finals-y2-s2/IRO/Summaries/Week%203%20-%20Nuclear%20Proliferation.md) — *"==**Nuclear taboo**=="*

---

### Sagan's three models + Waltz-Sagan debate + End of New START
*(A single argument about why proliferation is multicausal — and why the arms-control architecture is unravelling)*

**Sagan's three models (1996):** Why do states build nuclear weapons?
1. **Security model** — states build to deter external threats (realist logic). Cases: USSR vs US (1949), China vs US (1964), India vs China (1974/1998), Pakistan vs India (1998).
2. **Domestic politics model** — bureaucracies, military leaders, scientists, and politicians push for the bomb for parochial interests. Case: India 1974 (waited 10 years after China's 1964 test; Gandhi's domestic position explains the timing better than security does).
3. **Norms model** — nukes as symbols of national identity, modernity, and prestige. Case: France 1960 (de Gaulle: *"A France without world responsibility would be unworthy of herself"*).

Sagan's bottom line: *"Multicausality, rather than measurement error, lies at the heart of the nuclear proliferation problem."* Most cases mix all three models; pure-security explanations are rarely sufficient.

**Waltz-Sagan debate:**
- *Waltz ("more may be better"):* rational states under deterrence avoid war → spreading nukes extends caution. Evidence: no nuclear war since 1945; the Cold War "long peace"; India-Pakistan avoided direct war.
- *Sagan ("more will be worse"):* organisations have pathologies — near-accidents (Petrov 1983, Goldsboro 1961), civil-military dysfunction, irresponsible decision-makers. More nukes = higher catastrophic-failure risk. Mainstream IR has tilted toward Sagan since the early 2000s.

**End of New START (Feb 5, 2026):** New START (signed 2010) capped US and Russian deployed strategic warheads at 1,550 each. Russia suspended participation in 2023; the treaty expired on 5 February 2026 without renewal — the **first time in 50+ years with no binding limits on US and Russian nuclear arsenals**. Combined with the US withdrawal from INF (2019), the collapse of JCPOA (2018), and China's rapid arsenal expansion, this represents the most serious unravelling of the arms-control architecture since the Cold War.

**Connected to:** → [[#Security dilemma|security dilemma]] — the Waltz-Sagan debate is really about whether more proliferation makes the security dilemma better (Waltz) or worse (Sagan); → [[#constructivism + "where do those interests come from?"|constructivism]] (Week 2) — Sagan's norms model is constructivism applied to nuclear acquisition; → [[#Anthropocene + post-humanism|Anthropocene]] (Week 12) — nuclear risk is one of the planetary-scale challenges IR needs to engage.

**Seen in:**
- [Week 3 - Nuclear Proliferation.md:L27](projects/finals-y2-s2/IRO/Summaries/Week%203%20-%20Nuclear%20Proliferation.md) — *"==**Nuclear taboo**== — has held since Hiroshima and Nagasaki"*
- [Week 3 - Nuclear Proliferation.md:L39](projects/finals-y2-s2/IRO/Summaries/Week%203%20-%20Nuclear%20Proliferation.md) — *"==**End of New START (Feb 5, 2026)**=="*

---

## Week 4 — Non-Western IR Theory (Denney)

| Term | One-liner |
|---|---|
| NWIRT + Global IR | Twin labels; NWIRT diagnoses the West-centric default, Global IR is the constructive response |
| bandwagoning + balance of threat | Kang's claim Asia bandwagons vs. Walt's counter that states balance *threats*, not capabilities |
| double-binding | Acharya's alternative — enmesh both China and the US in regional institutions |
| constitutive localization + subsidiarity + deprovincialization | The three mechanisms of Global IR |
| Tianxia / tributary system / Mandala | Asia's pre-modern international orders — not Westphalian |
| twisted postcoloniality (S. Korea) | Why Korean IR scholarship is structurally trapped |
| Asian-origin global norms | ZOPFAN, nonintervention, human security, ASEAN regionalism |
| Asian exceptionalism trap | Tianxia / "Asian values" — replacing one parochialism with another |
| realism variants | Subaltern, neoclassical, defensive — making realism relevant beyond Europe |
| hegemonic acceptance (Gramscian) | The fifth structural barrier — internalised intellectual hegemony |

---

### NWIRT + Global IR

**NWIRT (Non-Western IR Theory):** An approach that argues mainstream IR theory (realism, liberalism, constructivism) is too rooted in Western history and intellectual tradition to capture non-Western experience. Calls for new IR built on **world history**, indigenous concepts, nationalist thought, and regional interaction patterns — not just using non-Western cases to test Western theories. (Acharya 2014)

**Global IR:** Acharya's preferred constructive label. **Not** a replacement for Western IR — a **broadening** of its sources and scope to make the discipline more inclusive and universal. Built on the recognition that Westphalian concepts are *one* historical tradition, not the universal default.

**Your question:** what is the difference between NWIRT and Global IR?
**Answer:** Think of NWIRT as the **diagnostic critique** and Global IR as the **constructive programme**. NWIRT is mostly a list of complaints about mainstream IR (Western-centric origins, missing Asian agency, treating non-West as testing-ground). Global IR is what Acharya proposes *instead*: don't reject Western IR — broaden it. So in practice you cite "NWIRT" when you want to name the critique and "Global IR" when you want to name Acharya's preferred solution. They're not rival schools; they're two sides of the same argument.

**Connected to:** → [[#constitutive localization + subsidiarity + deprovincialization|deprovincialization]] — Global IR's practical pathway; → [[#Asian historical orders — Tianxia / tributary / Mandala|Tianxia + tributary + Mandala]] — the pre-modern systems Western IR ignores.

**Seen in:**
- [[Week 4 - Non-Western IR Theory#^auto-non-western-ir-theory-l15|Week 4 - Non-Western IR Theory.md:L15]] — *"==**Non-Western IR (NWIRT)**=="*
- [[Week 4 - Non-Western IR Theory#^auto-non-western-ir-theory-l16|Week 4 - Non-Western IR Theory.md:L16]] — *"==**Global IR**== ... %%what is the difference between NWIRT and Global IR?%%"*

---

### bandwagoning + balance of threat

**Bandwagoning:** Aligning with the stronger or rising power rather than balancing against it. Logic: if you can't beat them, share in their gains. Kang argues Asian states historically did this with China (Sinic hierarchy with China at the centre, lesser rulers bandwagoning rather than balancing).

**Balance of threat (Walt):** States do not balance against raw *power* — they balance against *threats*, which depend on capability + geographic proximity + offensive intent + perception. This is why the US, despite being the most powerful state in the world, is not the one being balanced against — it isn't perceived as threatening by most allies.

**Your question:** what does bandwagoning mean in this context?
**Answer:** In the Friedberg–Kang–Acharya debate, **bandwagoning** is Kang's claim about how Asian states relate to a rising China — they fall in line behind it rather than forming a counter-coalition (which is what realist theory predicts). Acharya rejects this in two stages. (1) **Empirically**: Asian states aren't bandwagoning — India is balancing China, Japan's economic ties are about absolute gains and free-riding on US power (not deference), and ASEAN states are buying air/naval weapons. (2) **Conceptually**: the balancing-vs-bandwagoning binary is a Western (European balance-of-power) frame and misses what Asian states actually do — **double-binding**: enmesh both China *and* the US in regional institutions. So the answer to the question is layered: bandwagoning means "align with the dominant power," it's Kang's claim about Asia, and Acharya thinks neither the empirical claim nor the analytical frame holds.

**Connected to:** → [[#double-binding|double-binding]] — Acharya's proposed alternative to the balancing/bandwagoning dichotomy; → [[#NWIRT + Global IR|Kang's "Asian hierarchy" thesis]] — the thesis being refuted; → [[#Realism variants (subaltern / neoclassical / defensive)|realism variants]].

**Seen in:**
- [[Week 4 - Non-Western IR Theory#^auto-non-western-ir-theory-l91|Week 4 - Non-Western IR Theory.md:L91]] — *"==Asia has a tradition of **hierarchical order** ... states **bandwagon**== %%what does bandwagoning mean in this context?%%"*
- [[Week 4 - Non-Western IR Theory#^auto-non-western-ir-theory-l187|Week 4 - Non-Western IR Theory.md:L187]] — *"Walt's **balance of threat** theory shows states balance against *threats*"*

---

### double-binding

**Definition:** ASEAN's strategic posture: a conscious effort to **enmesh both China and the United States** in regional interdependence and institutions simultaneously, to (a) induce moderation from China and raise the cost of force, and (b) discourage US containment. Different from "hedging" (which is uncertainty); different from balancing or bandwagoning.

**Connected to:** → [[#bandwagoning + balance of threat|bandwagoning + balance of threat]] — Acharya's alternative to the binary; → [[#Asian-origin global norms — ZOPFAN / nonintervention / human security / ASEAN regionalism|ASEAN regionalism + ZOPFAN]].

**Seen in:**
- [[Week 4 - Non-Western IR Theory#^auto-non-western-ir-theory-l19|Week 4 - Non-Western IR Theory.md:L19]] — *"==**Double-binding**== ... %%Create an ASEAN file in the Geopolitics project...%%"*

---

### constitutive localization + subsidiarity + deprovincialization

These are the three mechanisms of **Global IR in practice**.

**Constitutive localization (Acharya 2004, 2009b):** A two-way process in which foreign ideas don't dominate or disappear — they **blend into the local normative setting**. Local actors actively adapt and reshape the foreign norm. Example: Asian states borrowed the idea of multilateralism but reshaped it via the "ASEAN Way" (consensus, non-interference).

**Subsidiarity (Acharya sense — NOT the EU sense):** The diffusion of **locally constructed rules and practices** transregionally or globally — used by local actors *"with a view to preserve their autonomy from dominance, neglect, violation, or abuse by more powerful central actors."* Example: nonalignment and human security — both Asian-origin concepts now global.

**Deprovincialization:** Acharya's preferred *third pathway* for IRT. Two moves: (1) stop using Westphalia/Europe as the universal benchmark; (2) avoid investigating Asia purely through its indigenous history. Localise Western IRT to fit Asia *and* universalise Asian constructs to a world stage.

**Your question:** so wait, Decentring is about provincializing and Acharya calls it deprovincialization but it's kind of the same thing that they argue? How do these to relate?
**Answer:** They look terminologically inverted but they're describing the *same intellectual move* from different angles. **Decentring** (Fisher-Onar & Nicolaïdis) starts from the European perspective and says "stop pretending Europe is the universal centre" → that's *provincialising* Europe. **Deprovincialization** (Acharya) starts from the Asian perspective and says "stop being trapped as a regional curiosity" → that's *universalising* Asia. Same movement, opposite endpoints in the metaphor — pull Europe down from "universal" to "one region among many," and lift Asia up from "area study" to "source of universal theory." On the exam: cite both labels, note they're complementary not rival, and use Decentring when discussing Week 5 / Fisher-Onar & Nicolaïdis, deprovincialization when discussing Acharya's NWIRT/Global IR proposal.

**Your question:** Are both of these happening at the same time or should it be like that or is it good/bad? Give more context.
**Answer:** Yes — both happen at once, and Acharya thinks that's how it *should* work. Constitutive localization is the *inward* flow (foreign ideas arriving in Asia, getting reshaped); subsidiarity is the *outward* flow (Asian-made ideas spreading to global governance). Together they describe a healthy two-way traffic of concepts: Asia isn't a passive recipient of Western norms (localization rules that out), but it's also not isolated and producing only local theory (subsidiarity rules that out either). The "good/bad" framing isn't quite the right axis — Acharya sees both flows as *empirical reality* (already happening: ASEAN reshaped multilateralism inward; human security flowed outward) and as *normatively desirable* (because they make IR theory more globally representative without forcing a choice between Western universalism and Asian exceptionalism).

**Connected to:** → [[#NWIRT + Global IR|NWIRT + Global IR]] — these are the *how* of Global IR; → [[#Asian-origin global norms — ZOPFAN / nonintervention / human security / ASEAN regionalism|Asian-origin global norms]] — concrete examples.

**Seen in:**
- [[Week 4 - Non-Western IR Theory#^auto-non-western-ir-theory-l17|Week 4 - Non-Western IR Theory.md:L17]] — *"==**Constitutive localization**=="*
- [[Week 4 - Non-Western IR Theory#^auto-non-western-ir-theory-l18|Week 4 - Non-Western IR Theory.md:L18]] — *"==**Subsidiarity (Acharya sense)**=="*
- [[Week 4 - Non-Western IR Theory#^auto-non-western-ir-theory-l27|Week 4 - Non-Western IR Theory.md:L27]] — *"==**Deprovincialization**== ... %%so wait, Decentring is about provincializing...%%"*

---

### Asian historical orders — Tianxia / tributary / Mandala

Three pre-modern international systems Western IR ignores. Important because they show Asia had its *own* concepts of order, none of which look Westphalian.

**Tributary system:** A pre-modern Chinese-centred international order (Tang to early Qing) based on **hierarchy** rather than sovereign equality: lesser rulers paid symbolic tribute to the Chinese emperor and received recognition. **Not consistently peaceful** — China used force against weaker neighbours where capable (Ming emperor Yongle invaded Vietnam; Zheng He intervened in Sumatra and Ceylon). Chinese restraint, where it occurred, was due to *lack of capability*, not benevolence.

**Mandala system:** The pre-modern Southeast Asian order of **overlapping circles of rulers** presiding over loosely organised states without clearly defined territorial limits. Authority diminished with distance from the king's centre — held together by personal hegemony, not bureaucracy. Indic (not Sinic) in inspiration.

**Tianxia ("all under heaven"):** A Chinese philosophical concept implying **harmony** (differentiated from sameness). Used by contemporary "Chinese School" scholars (Qin Yaqing and others) as the basis for a distinctively Chinese IR theory. Acharya is sceptical — calls it parochial and dangerously romantic about hierarchy.

**Your question:** describe more how the [tributary] system worked in practice.
**Answer:** In practice the tributary system worked as a ritual-and-recognition arrangement, not a tax-collection one. Lesser rulers (Korean kings, Vietnamese emperors, Ryukyuan sovereigns, Burmese kings) sent periodic envoys to the Chinese court bearing symbolic tribute — exotic goods, local products, sometimes elephants. In exchange they received the Chinese emperor's **investiture** (formal recognition of their legitimacy) plus return gifts often worth more than the tribute, plus the right to trade. The system was *symbolic-political*, not economic-extractive. Why it mattered: it provided **domestic legitimacy** (Korean kings cited Chinese recognition to settle succession disputes), **trade access** (only "tribute states" could trade through the Canton system), and **a hierarchical-but-stable regional order** as long as China was strong. When China weakened (late Qing), the system collapsed — and the Westphalian sovereign-state model rushed in to replace it via the 19th-century "unequal treaties." For the essay: the practical mechanism (ritual + recognition + trade access) is what makes it look different from European anarchy — it's hierarchy by *consent and ritual*, not by force or formal subordination.

**Connected to:** → [[#NWIRT + Global IR|NWIRT + Global IR]] — these orders are the historical evidence Acharya cites to show Asia is not Westphalian by default; → [[#Asian exceptionalism trap ("Asian values" / Tianxia)|Asian exceptionalism trap]] — Acharya warns against *romanticising* these systems (especially Tianxia).

**Seen in:**
- [[Week 4 - Non-Western IR Theory#^auto-non-western-ir-theory-l24|Week 4 - Non-Western IR Theory.md:L24]] — *"==**Tributary system**== ... %%Also relevant to china%%"*
- [[Week 4 - Non-Western IR Theory#^auto-non-western-ir-theory-l26|Week 4 - Non-Western IR Theory.md:L26]] — *"==**Tianxia ("all under heaven")**=="*
- [[Week 4 - Non-Western IR Theory#^auto-non-western-ir-theory-l61|Week 4 - Non-Western IR Theory.md:L61]] — *"**Chinese tributary system** ... %%describe more how the system worked in practice%%"*

---

### twisted postcoloniality (South Korea)

**Definition:** The South Korean case: a country **colonised by a non-Western power** (Japan, 1910–1945), then made dependent on the US after liberation. Result: Korean IR scholars are trapped between mimicking American IR (universality-focused, but loses identity) or remaining marginal (particularity-focused, but no global audience).

**Your question:** Why is there onlly these choices? Why is the colonization by a non-western power relevant?
**Answer:** Two reasons make the "only these two trap-choices" claim hold. First, **the structural incentives**: Korean academic promotion is tied to publishing in SSCI/Western journals (mimicry strategy), but Western journals reject as "area studies" anything that's too distinctively Korean (marginalisation strategy). The structural rules of the academic field force a binary. Second, **why the Japanese colonisation matters**: standard postcolonial theory (Said, Spivak) is built around *European* coloniser → *non-European* colonised. Korea was colonised by an *Asian* coloniser (Japan), which complicates the picture — there's no clean "West vs. rest" binary. After 1945 Korea then became *intellectually* colonised by the US (through alliance dependence, academic training, university models) — so the postcoloniality is "twisted": you have colonisation by a non-Western power *and* intellectual dependence on a Western one. The result is that Korean IR scholars can't simply reach for Said-style postcolonial critique (it doesn't quite fit Japan's role), nor can they reach for "indigenous" theory (since traditional Korean knowledge was rejected after liberation as backward). Both escape routes are structurally blocked.

**Connected to:** → [[#NWIRT + Global IR|NWIRT + Global IR]] — Korea is the *case study* showing that the barriers to non-Western IR are structural, not a lack of ideas; → [[#Hegemonic acceptance (Gramscian)|hegemonic acceptance (Gramscian)]] — the internalised acceptance of Western intellectual frames is what makes the trap stick.

**Seen in:**
- [[Week 4 - Non-Western IR Theory#^auto-non-western-ir-theory-l20|Week 4 - Non-Western IR Theory.md:L20]] — *"==**Twisted postcoloniality**== ... %%Why is there onlly these choices?%%"*

---

### Asian-origin global norms — ZOPFAN / nonintervention / human security / ASEAN regionalism

Concrete examples of **subsidiarity** in action — local norms that have diffused outward globally.

**Nonintervention / nonalignment:** Twin concepts championed by Asian and African states at the 1955 Bandung Conference; now foundational global norms (UN Charter Article 2(7), Non-Aligned Movement).

**ASEAN regionalism:** Consensus-based, sovereignty-respecting alternative to the EU model — *conservative and norm-preserving* rather than transformative. Non-interference is the central preserved norm.

**Human security:** Developed by Mahbub ul Haq (Pakistan) and Amartya Sen (India), then borrowed by Western donors (Canada, Norway) and adopted globally via UNDP. The classic Asian-origin-goes-global case.

**ZOPFAN (Zone of Peace, Freedom and Neutrality):** Southeast Asian initiative (1971) to keep the region free from great-power competition. Operationalised through ASEAN's later treaties.

**Connected to:** → [[#constitutive localization + subsidiarity + deprovincialization|constitutive localization + subsidiarity + deprovincialization]] — these are the example set; → [[#double-binding|double-binding]] — ZOPFAN is the historical precursor of ASEAN's double-binding strategy.

**Seen in:**
- [[Week 4 - Non-Western IR Theory#2.12 Global IR in practice (already happening)|Week 4 - Non-Western IR Theory.md:L137]] — *"==**Nonintervention / nonalignment**=="*
- [[Week 4 - Non-Western IR Theory#2.12 Global IR in practice (already happening)|Week 4 - Non-Western IR Theory.md:L138]] — *"==**ASEAN regionalism**=="*
- [[Week 4 - Non-Western IR Theory#^auto-non-western-ir-theory-l139|Week 4 - Non-Western IR Theory.md:L139]] — *"==**Human security** — Mahbub ul Haq=="*
- [[Week 4 - Non-Western IR Theory#^auto-non-western-ir-theory-l140|Week 4 - Non-Western IR Theory.md:L140]] — *"==**ZOPFAN**=="*

---

### Asian exceptionalism trap ("Asian values" / Tianxia)

**Definition:** The risk of building IR theory *purely* on Asian culture/history/regime preferences as a reaction against Western parochialism. Acharya's *path 2* — and he warns it just replaces one parochialism with another.

**Three concrete failure modes:**
- *"Asian values"* was used in the 1990s to justify authoritarian governance (Singapore, Malaysia).
- *Tianxia* + tributary nostalgia romanticise hierarchy and essentialise Chinese civilisation.
- It **homogenises "Asia"** — treats a vast, diverse continent as a single intellectual tradition.

**Connected to:** → [[#Asian historical orders — Tianxia / tributary / Mandala|Tianxia / tributary / Mandala]] — the historical material that gets weaponised when romanticised; → [[#constitutive localization + subsidiarity + deprovincialization|deprovincialization]] — Acharya's path 3, the way to avoid this trap.

**Seen in:**
- [[Week 4 - Non-Western IR Theory#2.10 The dangers of exceptionalism|Week 4 - Non-Western IR Theory.md:L118]] — *"=="Asian values" was used to justify authoritarian governance=="*
- [[Week 4 - Non-Western IR Theory#2.10 The dangers of exceptionalism|Week 4 - Non-Western IR Theory.md:L119]] — *"==**Tianxia** and tributary nostalgia romanticise hierarchy=="*

---

### Realism variants (subaltern / neoclassical / defensive)

**Subaltern realism (Ayoob):** Realism rebuilt around the *Third World state* — security challenges come not from external great powers but from internal state-building, post-colonial legitimacy crises, and weak institutions. Makes realism actually applicable to most of the world.

**Neoclassical realism:** Combines structural realism (system pressures) with classical realism's attention to *domestic-level intervening variables* (leader perception, regime type, state capacity). Explains why states with similar structural positions behave differently.

**Defensive realism:** Variant where states are assumed to seek *security* rather than power maximisation — explains restraint, cooperation, and security communities better than offensive realism (Mearsheimer).

**Why they matter for NWIRT:** Acharya cites these as evidence that Western IRT is *not monolithic* — there's space within realism for non-Western experience. Walt's *Origin of Alliances* used Middle East cases. These variants are part of why Acharya's argument is "broaden the discipline," not "replace it."

**Connected to:** → [[#bandwagoning + balance of threat|bandwagoning + balance of threat]] — Walt's framework is closest to defensive realism; → [[#NWIRT + Global IR|NWIRT + Global IR]] — Western IR is not monolithic.

**Seen in:**
- [[Week 4 - Non-Western IR Theory#^auto-non-western-ir-theory-l266|Week 4 - Non-Western IR Theory.md:L266]] — *"==**subaltern realism** (Ayoob), **neoclassical realism**, **defensive realism**=="*

---

### Hegemonic acceptance (Gramscian)

**Definition:** The fifth structural barrier to NWIRT: non-Western scholars **internalise the assumption** that Western theory is "better." Operates through consent rather than coercion — the kind of hegemony Antonio Gramsci described.

**Why it matters:** Combined with the other four structural barriers (institutional, policy pull, language, area studies / IR divide), this explains why NWIRT has been slow to develop even where the material conditions are present — and why "just demand more non-Western scholars publish" misses the point.

**Connected to:** → [[#twisted postcoloniality (South Korea)|twisted postcoloniality]] — the Korean case shows how Gramscian acceptance becomes a survival strategy; → [[#multiplex world order + multipolarity distinction|hegemonic stability theory]] — different "hegemony" concept, both relevant on the exam.

**Seen in:**
- [[Week 4 - Non-Western IR Theory#^auto-non-western-ir-theory-l28|Week 4 - Non-Western IR Theory.md:L28]] — *"==**Hegemonic acceptance (Gramscian sense)**=="*

---

## Week 6 — Global Migration: Mobility and Security

| Term | One-liner |
|---|---|
| Hobson's choice | Apparent choice with only one real option |
| liberal paradox | States trapped between economic openness and political closure |
| migration state | Third state function: regulating who can enter and stay |
| internal market spillover into internal security | How the single market created EU security integration |
| security continuum | Institutional field linking migration, terrorism, crime |
| asylum shopping | Applying in multiple countries; Dublin was the fix |

---

### Hobson's choice

**Definition:** A Hobson's choice is an apparent free choice where in reality there is only one option — take it or leave it. The phrase comes from Thomas Hobson (1544–1631), a Cambridge stable owner who required customers to take the horse nearest the door or none at all.

**In Week 6 context:** Hollifield uses it to describe the state's trap around irregular migration: tolerate illegal immigration and a black-market labour economy, or try to enforce strict controls and watch courts repeatedly block you. Neither option "works" — hence Hobson's choice.

**Connected to:** → [[#liberal paradox + migration state|liberal paradox]] — Hobson's choice is the street-level symptom; the liberal paradox is the structural explanation for why the trap exists.

**Seen in:**
- [[Week 6 - Global Migration - Mobility and Security#^auto-global-migration-mobility-and|Week 6 - Global Migration - Mobility and Security.md:L374]] — *"illegal immigration and a growing black market for labour — 'a ==Hobson's choice=='"*

---

### liberal paradox + migration state
*(Hollifield — these two concepts form a single argument)*

**liberal paradox:** Liberal states face a structural contradiction between their **economic logic** (openness to trade, investment, and migration is necessary for growth and competitiveness) and their **political/legal logic** (territorial closure is necessary for democracy, social contract, and public legitimacy). Courts protect migrants once they're inside; public opinion demands border control. States are permanently caught between both pulls and cannot fully satisfy either.

**migration state:** Hollifield's name for the third core function of the modern state, alongside the garrison state (security) and the trading state (economic growth). In the migration state, deciding who can enter, stay, and gain citizenship becomes as central to state purpose as defence or trade policy.

**How they connect:** The migration state is Hollifield's proposed *solution* (or at least, his diagnosis of what states are already doing) to the liberal paradox. Once you accept that migration cannot simply be stopped or fully opened, you accept that managing it is a permanent core function of government — that's the migration state. The EU is the only experiment so far in doing this *collectively* across multiple liberal states, which lets member states "finesse" (not escape) the paradox by offloading some of it to the regional level.

**Explained easily:** Germany invited Turkish guest workers in the 1960s → tried to send them home after 1973 → courts said family reunification is a right → permanent immigrant population formed despite the policy reversal. That's the paradox. The 1999 *jus soli* reform and the 2000 Green Card for tech workers both showed the same pattern: you can't import workers without importing humans. A government that acknowledges this and builds institutions around managing it is a migration state.

**Connected to:** → [Hobson's choice](#hobsons-choice) for the street-level expression of the paradox; → [[#security continuum + internal market spillover|security continuum]] for the rival (Huysmans) explanation of why migration becomes political.

**Seen in:**
- [[Week 6 - Global Migration - Mobility and Security#^auto-global-migration-mobility-and-2|Week 6 - Global Migration - Mobility and Security.md:L384]] — *"naming this trap — the ==**liberal paradox**=="*
- [[Week 6 - Global Migration - Mobility and Security#^auto-global-migration-mobility-and-3|Week 6 - Global Migration - Mobility and Security.md:L498]] — *"==Liberal paradox: economic openness × political closure.=="*
- [[Week 6 - Global Migration - Mobility and Security#^auto-global-migration-mobility-and-4|Week 6 - Global Migration - Mobility and Security.md:L499]] — *"==Three state functions: garrison + trading + **migration state**=="*

**Sources:** Hollifield (1992, 2004) *Immigrants, Markets and States*; see also Week 6 summary sections 4.13 and 6.3.

---

### security continuum + internal market spillover
*(Huysmans — these two concepts explain the same phenomenon from different angles)*

**security continuum:** Bigo and Huysmans' term for the institutionalised governance field that links border control, terrorism, transnational crime, and migration into a single domain. Once these issues share institutions — the TREVI network, Schengen, the Third Pillar — the security connotations of terrorism transfer to migration. This is a structural feature of EU policy, produced by bureaucratic integration, not merely by political rhetoric.

**internal market spillover into internal security:** Huysmans' specific mechanism explaining *how* the security continuum formed in the EU. Creating the single internal market (Single European Act 1986) required abolishing internal border checks → which required compensating external border measures → which required common visa policy, data sharing (Eurodac, SIS), and police cooperation → which bundled migration with terrorism and crime in the same institutions. Migration became a security issue through integration logic, not just political choices.

**How they connect:** "Internal market spillover" is the *historical explanation* for how the security continuum came to exist in Europe. The SEA created functional pressure that led, step by step, to migration and terrorism sharing the same governance structures. The security continuum is the *resulting structure* — the ongoing institutional reality where migration is permanently coded as a security issue because it's governed alongside terrorism and crime.

**Explained easily (the key difference from Copenhagen School):** The Copenhagen School (Wæver) says securitisation happens when a politician makes a speech declaring something a security threat. Huysmans/Paris School says: look at the *border guard at the airport*, the *customs officer profiling travellers*, the *Frontex database*. These everyday professional practices build the security frame from the bottom up, through shared institutions — not through speeches. The security continuum is that institutional reality.

**Connected to:** → [[#liberal paradox + migration state|liberal paradox + migration state]] — Huysmans and Hollifield are **complementary**: Hollifield tells you *why states are economically stuck* with migration (the paradox); Huysmans tells you *how migration gets politically constructed as a security threat* (the continuum). You need both for a strong essay answer.

**Seen in:**
- [[Week 6 - Global Migration - Mobility and Security#^auto-global-migration-mobility-and-5|Week 6 - Global Migration - Mobility and Security.md:L449]] — *"==**Security continuum** (Bigo / Huysmans) — institutionalised mode of policy-making linking border control, terrorism, crime and migration=="*
- [[Week 6 - Global Migration - Mobility and Security#^auto-global-migration-mobility-and-6|Week 6 - Global Migration - Mobility and Security.md:L506]] — *"The EU ==**internal market spillover into internal security**==: SEA → Schengen → Third Pillar → Dublin → Amsterdam"*
- [[Week 6 - Global Migration - Mobility and Security#^auto-global-migration-mobility-and-7|Week 6 - Global Migration - Mobility and Security.md:L461]] — *"Police and security professionals — via networks like TREVI and Bern Club — produced a ==**security continuum**=="*

**Sources:** Huysmans (2000) *The European Union and the Securitization of Migration*; Bigo (1994).

---

### asylum shopping

**Definition:** The practice (alleged or real) of asylum-seekers choosing between countries based on which offers the most favourable conditions — higher recognition rates, better benefits, faster processing. The term is politically loaded because it implies migrants are strategic actors rather than people fleeing persecution.

**The Dublin fix (and its problems):** The Dublin Convention (1990) was designed to end asylum shopping by requiring asylum seekers to apply in the *first safe country* of arrival. In practice this shifted the burden onto southern border states (Greece, Italy) that first-arrivals land in, and was largely suspended after 2015. Critics argue "asylum shopping" misrepresents refugees' actual situation — most have little choice in which country they reach first.

**Connected to:** → [[#security continuum + internal market spillover|security continuum]] — Dublin is one of the legal instruments produced by the Schengen/Third Pillar integration chain that Huysmans analyses. The "asylum shopping" discourse also feeds into the security continuum by framing refugees as strategic and therefore suspect actors.

**Seen in:**
- [[Week 6 - Global Migration - Mobility and Security#^auto-global-migration-mobility-and-8|Week 6 - Global Migration - Mobility and Security.md:L455]] — *"Dublin Convention (1990) — asylum seekers must apply in first 'safe country' of arrival; prevents ==asylum shopping=="*

---

## Week 7 — War, Peace, and International Security (Wolff)

| Term | One-liner |
|---|---|
| Clausewitz on war | War as continuation of politics by other means |
| security communities (Deutsch) | Where war is "unthinkable" — EU/NATO area |
| Just War Theory (ad bellum / in bello / post bellum) | Middle ground: war can be morally justified under conditions |
| new wars (Kaldor) | Post-Cold War: blur war/crime, non-state actors, identity-driven |
| decolonising war + small wars (Barkawi) | The Eurocentric concept of war excludes colonial conflicts |
| R2P | Sovereignty conditional on protecting population (2005 UN) |
| Private Military Security Industry (PMSI) | Commercial firms; 1 in 4 US personnel in Iraq/Afghanistan |
| democratic peace theory | Liberal democracies almost never fight each other |
| "Dragon" states (Kilcullen) | Russia, China — refused US order; three warfares |
| ReArm Europe / SAFE | EU's €800B 2025 defence response to Ukraine |

---

### Clausewitz on war

**Definition:** *"War is simply a continuation of political intercourse, with the addition of other means."* War and politics are co-constitutive — there is no sharp dividing line between them. The foundational realist statement on war.

**Connected to:** → [[#decolonising war + small wars (Barkawi)|decolonising war + small wars]] — Barkawi accepts the Clausewitz formula but argues IR has applied it selectively to European interstate wars only; → [[#Just War Theory (jus ad bellum + in bello + post bellum)|Just War Theory]] — JWT is the moral middle ground between Clausewitz (war as legitimate political tool) and pacifism.

**Seen in:**
- [[Week 7 - War Peace and International Security#^auto-war-peace-and-international-se|Week 7 - War Peace and International Security.md:L33]] — *"==**Clausewitz on war**=="*

---

### security communities (Deutsch)

**Definition:** Karl Deutsch (1957): groups of states where war is "unthinkable" because of shared identity and institutions — e.g. the EU/NATO area.

**Connected to:** → [[#democratic peace theory|democratic peace theory]] — both are liberal claims about how war becomes implausible between certain states; → [[#ReArm Europe / SAFE|ReArm Europe / SAFE]] — the Ukraine war tests whether the EU security community holds at its borders.

**Seen in:**
- [[Week 7 - War Peace and International Security#^auto-war-peace-and-international-se-2|Week 7 - War Peace and International Security.md:L37]] — *"==**Security communities**=="*

---

### Just War Theory (jus ad bellum + in bello + post bellum)

**Definition:** The idea that war can be morally justified if certain ethical conditions are met. Divided into:
- **Jus ad bellum** — *right to go to war*: last resort, just cause, legitimate authority, right intention, reasonable prospect of success, proportionality.
- **Jus in bello** — *right conduct during war*: discrimination (combatants vs. civilians), proportionality, humanity.
- **Jus post bellum** — *justice after war*: terms of peace, accountability, reconstruction.

JWT sits between realism (war as legitimate political tool) and pacifism (war never legitimate).

**Explained easily:** Just war is like the conditions for a legitimate fistfight — you can only punch someone if they hit you first (just cause), you tried talking it out (last resort), you don't hit harder than necessary (proportionality), and you don't go for the eyes (discrimination). It tries to distinguish "tragically necessary" wars from aggression.

**Connected to:** → [[#R2P (Responsibility to Protect)|R2P]] — R2P is a contemporary doctrine that builds on JWT's *jus ad bellum*; → [[#Clausewitz on war|Clausewitz on war]] — JWT's moral framework is the *other* answer to Clausewitz's realism.

**Seen in:**
- [[Week 7 - War Peace and International Security#^auto-war-peace-and-international-se-3|Week 7 - War Peace and International Security.md:L38]] — *"==**Just War Theory**=="*
- [[Week 7 - War Peace and International Security#^auto-war-peace-and-international-se-4|Week 7 - War Peace and International Security.md:L40]] — *"==proportionality==" (jus in bello)*

---

### new wars (Kaldor)

**Definition:** Post-Cold War conflicts that blur the line between war and crime, involve non-state actors, and are fought over identity rather than ideology. Associated with Mary Kaldor.

**Connected to:** → [[#decolonising war + small wars (Barkawi)|decolonising war + small wars]] — Barkawi's critique applies: are "new wars" *new*, or have they always been the norm for non-European states? → [[#"Dragon" states (Kilcullen)|"Dragon" states]] — Kilcullen's update for great-power competition + non-state threats running in parallel.

**Seen in:**
- [[Week 7 - War Peace and International Security#^auto-war-peace-and-international-se-5|Week 7 - War Peace and International Security.md:L41]] — *"==**New wars**=="*

---

### decolonising war + small wars (Barkawi)

**Decolonising war:** Critiquing how Eurocentric ideas have shaped the very category of "war." Barkawi's three Eurocentric building blocks to challenge: (1) war/peace as opposites — erases continuous colonial force; (2) war happens between sovereign states — excludes empire vs. non-state insurgents; (3) war is either international or civil — cannot capture colonial wars.

**Small wars:** Colonial military operations (insurgencies, uprisings, police actions) that Eurocentric war studies dismissed as "not real war" — but which are central to Barkawi's decolonial project.

**Barkawi's alternative:** Conceive force as an *ordinary* dimension of politics (not an exception), situate war in transnational/imperial context, attend to how war and society are *co-constitutive*.

**Explained easily:** Imagine if historians only defined "music" as classical orchestral music, and put hip-hop, jazz, and folk into separate lesser categories called "sounds." That's what Eurocentric war studies did with colonial conflicts — they defined "real war" as European interstate war, and called everything else (uprisings, counterinsurgency, occupations) something other than war. Barkawi: the definition is the problem. Fix the concept, not just the geography.

**Connected to:** → [[#Clausewitz on war|Clausewitz on war]] — Barkawi accepts Clausewitz's politics-and-war fusion but charges IR with applying it selectively; → [[#NWIRT + Global IR|NWIRT + Global IR]] (Week 4) — Barkawi extends the same decolonial logic to war studies that Acharya extends to IR theory broadly.

**Seen in:**
- [[Week 7 - War Peace and International Security#^auto-war-peace-and-international-se-6|Week 7 - War Peace and International Security.md:L42]] — *"==**Decolonising war** (Barkawi)=="*
- [[Week 7 - War Peace and International Security#^auto-war-peace-and-international-se-7|Week 7 - War Peace and International Security.md:L43]] — *"==**Small Wars**=="*

---

### R2P (Responsibility to Protect)

**Definition:** 2005 UN doctrine: sovereignty is conditional on a state protecting its population; if it fails, the international community has a responsibility to intervene.

**Connected to:** → [[#Just War Theory (jus ad bellum + in bello + post bellum)|Just War Theory]] — R2P operationalises *jus ad bellum* "just cause" (atrocity prevention); → [[#decolonising war + small wars (Barkawi)|decolonising war]] — critical view: R2P can become a *technology of colonial-modern rule* (Shilliam 2013).

**Seen in:**
- [[Week 7 - War Peace and International Security#^auto-war-peace-and-international-se-8|Week 7 - War Peace and International Security.md:L45]] — *"==**R2P (Responsibility to Protect)**=="*

---

### Private Military Security Industry (PMSI)

**Definition:** Commercial companies (PMSCs) that provide armed security and military services to states, NGOs, and corporations. Expanded massively after the Cold War and 9/11. By 2016, **1 in 4 US armed personnel in Iraq/Afghanistan was a private contractor**. The Montreux Document (2008) attempted some legal framework.

**Why it matters:** Raises questions about accountability, sovereignty, and democratic control of force — if a fourth of "your army" is privately contracted, who is the principal in the principal-agent relationship?

**Connected to:** → [[#Clausewitz on war|Clausewitz on war]] — PMSI complicates the "state monopoly on legitimate violence" assumption built into the realist account; → [[#multiplex world order + multipolarity distinction|neoconservatism (Kagan post-9/11)]] (Week 8) — PMSI expansion is part of the post-9/11 US unilateral primacy project.

**Seen in:**
- [[Week 7 - War Peace and International Security#^auto-war-peace-and-international-se-9|Week 7 - War Peace and International Security.md:L46]] — *"==**Private Military Security Industry (PMSI)**=="*

---

### democratic peace theory

**Definition:** Empirical claim that liberal democracies do not fight each other — a key liberal IR argument. (Deudney & Ikenberry 1999) Explanation: shared norms (respect for rights, rule of law) and economic interdependence make war too costly and morally illegitimate between democracies.

**Connected to:** → [[#security communities (Deutsch)|security communities (Deutsch)]] — closely related liberal claim about where war becomes unthinkable; → [[#multiplex world order + multipolarity distinction|globalization trilemma]] (Week 8) — DPT was the optimistic post-Cold War expectation; Rodrik's trilemma + populist backlash explains the souring.

**Seen in:**
- [[Week 7 - War Peace and International Security#^auto-war-peace-and-international-se-10|Week 7 - War Peace and International Security.md:L70]] — *"==Liberals see war as a problem solvable by spreading democracy (*democratic peace theory*)=="*

---

### "Dragon" states (Kilcullen)

**Definition:** ==Russia and China — states that *refused* to orbit the US-led order and developed alternative strategies, including *"three warfares"* (psychological warfare, public opinion warfare, "lawfare"). Kilcullen's category, contrasted with "Snakes" (non-state actors and threats from state collapse).==

**Connected to:** → [[#multiplex world order + multipolarity distinction|multiplex world order]] (Week 8) — Acharya's diversity-of-actors framing fits Kilcullen's "Dragons + Snakes" descriptive picture; → [[#decolonising war + small wars (Barkawi)|decolonising war]] — both Dragons and Snakes break the European interstate-war template.

**Seen in:**
- [[Week 7 - War Peace and International Security#New Wars and Contemporary Conflict|Week 7 - War Peace and International Security.md:L111]] — *"=="Dragon" states (Russia, China)... including "three warfares"=="*

---

### ReArm Europe / SAFE

**Definition:** The EU's €800 billion *ReArm Europe* plan (2025): suspends strict budget rules, pools procurement, and creates a new lending instrument (**SAFE**). A direct consequence of the Ukraine war and a historic shift in European defence posture.

**Connected to:** → [[#security communities (Deutsch)|security communities (Deutsch)]] — ReArm tests whether the EU is also a security community *vis-à-vis its periphery*, not just internally; → [[#multiplex world order + multipolarity distinction|multiplex world order]] (Week 8) — Europe becoming a more autonomous pole is part of the move from US-led unipolarity to multiplexity.

**Seen in:**
- [[Week 7 - War Peace and International Security#Re-Arm Europe (contemporary relevance)|Week 7 - War Peace and International Security.md:L120]] — *"==pooling procurement==" / "==new lending instrument (SAFE==)"*

---

## Week 8 — Power, Liberalism, and World Order (Wolff)

| Term | One-liner |
|---|---|
| world order (Heywood / Roach & O'Callaghan) | Distribution of power giving rise to stable patterns; not necessarily peace or justice |
| multiplex world order + multipolarity distinction | Acharya — diversity of actors/norms; "multiple shows" — not just more poles |
| hard / soft / smart power (Nye) | Coerce / attract / combine |
| power as resource vs. outcome | What you possess (realist) vs. what you can achieve relationally |
| institutional power | Power embedded in the rules of the game (UN/WTO/IMF) |
| Belt and Road Initiative (BRI) | Xi's bilateral infrastructure programme; "debt trap" criticism |
| BRICS + Beijing Consensus | Alternative grouping and alternative development model |
| neoconservatism (Kagan post-9/11) | US asserted unilateral primacy; undermined the multilateral order it built |
| globalization trilemma (Rodrik) | Deep globalization + nation-state + democracy — pick any two |
| functionalism + neofunctionalism + spillover | EU integration theories — Mitrany → Haas |
| intergovernmentalism + liberal intergovernmentalism | States as key actors — Hoffmann → Moravcsik |
| comparative regionalism | Studying ASEAN, AU, etc. alongside (not below) the EU |

---

### world order

**Definition:** The actual or perceived distribution of power between and amongst states and other key actors, giving rise to a relatively stable pattern of relationships and behaviours (Heywood). Roach & O'Callaghan add the caution: *not to be confused with peace or justice.* A stable world order can be deeply unjust.

**Two organising nexuses:**
- **Order and power:** order follows the distribution of power (the liberal order was a product of US dominance).
- **Order and diversity:** the central question today — how do you sustain a stable order when values, interests, and power are increasingly diverse?

**Connected to:** → [[#multiplex world order + multipolarity distinction|multiplex world order]] — Acharya's bet on diversity; → [[#multiplex world order + multipolarity distinction|neoconservatism (Kagan)]] — the US unilateralism that fractured the post-1991 liberal order.

**Seen in:**
- [[Week 8 - Power Liberalism and World Order#What is world order and why does it matter?|Week 8 - Power Liberalism and World Order.md:L48]] — *"==The lecture opens with two definitions of world order=="*
- [[Week 8 - Power Liberalism and World Order#^auto-power-liberalism-and-world-ord|Week 8 - Power Liberalism and World Order.md:L50]] — *"==**Roach & O'Callaghan:** a stable pattern of relations... not to be confused with peace or justice.=="*

---

### multiplex world order + multipolarity distinction
*(Acharya — also tied to neoconservatism / Kagan + hegemonic stability theory)*

**Multiplex world order:** Acharya's concept — a world order with "multiple shows on offer, giving the audience a choice of producers, directors, actors and plots." No single power sets the rules; *diversity itself* is the organizing principle.

**Crucial distinction from multipolarity:** Multipolarity describes *how many* powers there are (2 = bipolar, 3+ = multipolar). Multiplexity describes the *character* of the system: diverse, with no single actor able to set universal rules. **A cinema with multiple screens, not just a bigger screen.**

**Neoconservatism (Kagan, post-9/11):** *"America did not change on September 11. It only became more itself."* → US asserted unilateral primacy, undermining the multilateral rules-based order it had built. The Bush Doctrine: preemptive war, democracy promotion by force (Iraq 2003).

**Hegemonic stability theory:** A single dominant power stabilises the international system by providing public goods (security, trade rules, financial stability). Ikenberry restates this when he argues the "American-led liberal hegemonic order" can accommodate China. Acharya criticises this view as ignoring Asian agency.

**Explained easily — world orders:** Think of the international system like the music industry. Cold War (bipolar) = two major labels. Post-1991 (unipolar) = one label (US-led liberal order) dominates. Now (multipolar/multiplex) = streaming era — many labels, many genres, audiences choose. Acharya's multiplex is the streaming era of world politics.

**Connected to:** → [[#NWIRT + Global IR|NWIRT + Global IR]] (Week 4) — multiplex is Global IR applied to world order; → [[#BRI + BRICS + Beijing Consensus|BRI + BRICS + Beijing Consensus]] — concrete alternative orders being built; → [[#NWIRT + Global IR|Acharya, Amitav]] — same author across Weeks 4, 8, 12.

**Seen in:**
- [[Week 8 - Power Liberalism and World Order#^auto-power-liberalism-and-world-ord-2|Week 8 - Power Liberalism and World Order.md:L23]] — *"==**Multiplex world order**=="*
- [[Week 8 - Power Liberalism and World Order#^auto-power-liberalism-and-world-ord-3|Week 8 - Power Liberalism and World Order.md:L68]] — *"==Acharya's *multiplex* is importantly different from simple *multipolarity*==..."*
- [[Week 8 - Power Liberalism and World Order#^auto-power-liberalism-and-world-ord-4|Week 8 - Power Liberalism and World Order.md:L206]] — *"==Neoconservatism — the US asserted unilateral primacy=="*

---

### hard / soft / smart power (Nye)

**Hard power:** Military and economic power — coerce through force, sanctions, or payments. Realist tool.

**Soft power:** Ability to attract and co-opt through culture, political ideals, and foreign policies — *without* coercion. Liberal tool.

**Smart power:** Combining hard and soft power strategically.

**Explained easily:** Hard power is getting what you want by threatening or paying someone — like a bouncer. Soft power is getting what you want because people admire you and want to be like you — like a musician whose fans copy their style. The US has both: its military (hard) + Hollywood/universities/democracy (soft). China has been building hard power for decades and is now actively trying to develop soft power (BRI, cultural diplomacy).

**Connected to:** → [[#power as resource vs. outcome|power as resource vs. outcome]] — Nye's typology is mostly *resource*; the resource/outcome distinction is *orthogonal* (Joseph Nye on what kinds of resources; Robert Dahl on what counts as influence); → [[#BRI + BRICS + Beijing Consensus|BRI]] — BRI is the canonical case of hard + soft power combined.

**Seen in:**
- [[Week 8 - Power Liberalism and World Order#^auto-power-liberalism-and-world-ord-5|Week 8 - Power Liberalism and World Order.md:L29]] — *"==**Smart power**=="*

---

### power as resource vs. outcome

**Power as resource (absolute/quantitative):** about what you *possess* — military might, economic size, population. Associated with realism.

**Power as outcome (relational):** about your ability to *use* what you have, including through skills, strategies, and perceptions. Even small states can be influential this way.

**Why the distinction matters:** Two states with similar resources can have very different *outcomes* depending on their relational capacity (alliances, soft power, institutional positioning). This is why states like Singapore or the Netherlands punch above their weight.

**Connected to:** → [[#hard / soft / smart power (Nye)|hard/soft/smart power]] — Nye's typology is mostly resource; the outcome dimension is what makes soft power *work*; → [[#institutional power|institutional power]] — relational + structural channel.

**Seen in:**
- [[Week 8 - Power Liberalism and World Order#^auto-power-liberalism-and-world-ord-6|Week 8 - Power Liberalism and World Order.md:L76]] — *"==The lecture distinguishes two types of power=="*

---

### institutional power

**Definition:** Power is embedded in institutions (Thelen 2004). Institutions are *"the rules of the game"* (North 1990) — whoever shapes the rules shapes outcomes. The US shaped post-WWII institutions (UN, WTO, IMF) in its own image, giving it **structural/institutional power long after its material dominance peaked**.

**Connected to:** → [[#multiplex world order + multipolarity distinction|hegemonic stability theory]] — institutional power is the *mechanism* by which a fading hegemon retains influence; → [[#BRI + BRICS + Beijing Consensus|BRI + BRICS]] — these are China's attempt to build *parallel* institutions (because the existing ones are US-shaped).

**Seen in:**
- [[Week 8 - Power Liberalism and World Order#^auto-power-liberalism-and-world-ord-7|Week 8 - Power Liberalism and World Order.md:L85]] — *"==**Institutional power:** Power is embedded in institutions==..."*

---

### BRI + BRICS + Beijing Consensus

**Belt and Road Initiative (BRI, 2013):** Xi Jinping's signature foreign policy initiative — a massive series of *bilateral* infrastructure investment and lending agreements linking China to its neighbours across Asia, Africa, and Europe. **Not a multilateral institution.** Funded via the China Development Bank. Criticised as "debt trap diplomacy."

**BRICS:** Brazil, Russia, India, China, South Africa — a grouping of major emerging economies, increasingly seen as an alternative to Western-led institutions (G7/G8).

==**Beijing Consensus:** China's alternative development model — **development without liberal democracy**.== Contrasts with the Washington Consensus (market liberalisation + democratisation as a package).

**Why these matter for world order:** Together they sketch what an alternative-to-US-led order could look like. Not a single replacement hegemony, but a *parallel* set of institutions and norms that give middle powers options outside the Western-dominated system. This is part of why Acharya's *multiplex* (rather than simple multipolarity) is the better description: it's about diversity of *institutional menus*, not just diversity of great powers.

**Connected to:** → [[#multiplex world order + multipolarity distinction|multiplex world order]] — these are the concrete content of multiplexity; → [[#institutional power|institutional power]] — BRI/BRICS are China's bid to shape the rules of the game.

**Seen in:**
- [[Week 8 - Power Liberalism and World Order#^auto-power-liberalism-and-world-ord-8|Week 8 - Power Liberalism and World Order.md:L232]] — *"==Acharya's multiplex world order... Chinese "Beijing Consensus"=="*
- [[Week 8 - Power Liberalism and World Order#^auto-power-liberalism-and-world-ord-9|Week 8 - Power Liberalism and World Order.md:L234]] — *"==Belt and Road Initiative as China's alternative... BRICS as an alternative grouping=="*

---

### globalization trilemma (Rodrik)
*(Appears in Week 8 introduction; carries through Week 11; reappears in Week 12. One of the load-bearing concepts of the course.)*

**Definition:** Rodrik's "impossibility theorem": **democracy + national sovereignty + deep global economic integration** are mutually incompatible. You can only fully have any two of the three simultaneously.

**Important nuance** (introduced in Week 11): *"deep"* globalization means **regulatory harmonization across jurisdictions**, NOT aggregate trade volume. A country can trade a lot and still stay on the nation-state-plus-democracy corner. Brexit Britain still trades plenty — what it gave up was regulatory harmonization.

**The three corners + sacrifices:**
| You keep | You sacrifice | Empirical case |
|---|---|---|
| Globalization + democracy | National policy autonomy | EU (technocratic delegation; Greek 2015 referendum) |
| Globalization + nation-state | Democracy | China; the "Golden Straitjacket" pattern |
| Nation-state + democracy | Depth of globalization | Brexit UK; US under Trump |

**Explained easily:** Imagine three things you want at full strength: (1) a fully open global economy where capital and goods flow freely, (2) your country making its own rules, (3) your country being democratic. You can pick any two. The EU picked open economy + democracy but gave up sovereignty (Brussels makes the rules). China picked open economy + sovereignty but skipped democracy. Brexit Britain picked sovereignty + democracy but closed off the open economy. There's no fourth option where you get all three — somebody, somewhere, is paying for what you chose.

**Connected to:** → [[#populism: demand vs supply + populism as repertoire vs nationalism|populism: demand vs supply]] (Week 11) — the trilemma generates the *demand* for populism, but not its political form; → [[#precariat (Standing)|precariat]] — the bloc that bears the cost of the chosen sacrifice; → [[#Anthropocene + post-humanism|Anthropocene]] (Week 12) — Denney noted in Week 12 that the trilemma reappears with planetary boundaries replacing democracy as the squeezed third corner.

**Seen in:**
- [[Week 8 - Power Liberalism and World Order#^auto-power-liberalism-and-world-ord-10|Week 8 - Power Liberalism and World Order.md:L30]] — *"==**Globalization trilemma**=="*
- [[Week 8 - Power Liberalism and World Order#^auto-power-liberalism-and-world-ord-11|Week 8 - Power Liberalism and World Order.md:L142]] — *"==Imagine three things you want...=="*
- [[Week 11 - Globalization and Populism#^auto-globalization-and-populism-l28|Week 11 - Globalization and Populism.md:L28]] — *"==**Globalization trilemma**== ... Rodrik (2011, 2018)"*

---

### functionalism + neofunctionalism + spillover
*(EU integration theories — read together)*

**Functionalism (Mitrany):** States integrate in technical/functional areas (trade, transport) first; political integration follows. The functional logic *drives* the political logic.

**Neofunctionalism (Haas):** Update — integration generates a **"spillover" effect**: success in one sector creates pressure to integrate adjacent sectors, eventually drawing in political authority.

**Spillover:** The dynamic itself. *Example*: a single market requires common competition rules; common competition rules require shared enforcement; shared enforcement requires supranational courts; etc.

==**Why this matters across the course:** Spillover is exactly the mechanism Huysmans uses in Week 6 to explain how the EU's *internal* market spilled into *internal security* (the security continuum). Same logic, security domain.==

**Connected to:** → [[#security continuum + internal market spillover|internal market spillover into internal security]] (Week 6) — Huysmans' mechanism is a neofunctionalist spillover applied to security; → [[#intergovernmentalism + liberal intergovernmentalism|intergovernmentalism + liberal intergovernmentalism]] — the rival theory.

**Seen in:**
- [[Week 8 - Power Liberalism and World Order#^auto-power-liberalism-and-world-ord-12|Week 8 - Power Liberalism and World Order.md:L34]] — *"==**Functionalism**=="*
- [[Week 8 - Power Liberalism and World Order#^auto-power-liberalism-and-world-ord-13|Week 8 - Power Liberalism and World Order.md:L35]] — *"==**Neofunctionalism**=="*

---

### intergovernmentalism + liberal intergovernmentalism

==**Intergovernmentalism (Hoffmann):** Theory of integration where **national governments remain the key actors**; states cooperate only when it serves national interests. Rival to functionalism — institutions are servants of state preferences, not autonomous drivers.==

==**Liberal intergovernmentalism (Moravcsik):** Refinement — EU integration is driven by **bargains between major member states**, which in turn reflect *domestic economic preferences*. The synthesis explains both why states integrate (mutual gains from policy coordination) and why integration stalls (when domestic preferences diverge).==

**Connected to:** → [[#functionalism + neofunctionalism + spillover|functionalism + neofunctionalism]] — the rival theory; → [[#comparative regionalism|comparative regionalism]] — the question whether *any* of these EU-derived theories explain ASEAN, AU, etc.

**Seen in:**
- [[Week 8 - Power Liberalism and World Order#^auto-power-liberalism-and-world-ord-14|Week 8 - Power Liberalism and World Order.md:L37]] — *"==**Intergovernmentalism**=="*
- [[Week 8 - Power Liberalism and World Order#^auto-power-liberalism-and-world-ord-15|Week 8 - Power Liberalism and World Order.md:L38]] — *"==**Liberal intergovernmentalism**=="*

---

### comparative regionalism

**Definition:** The study of regional integration arrangements **beyond Europe** — ASEAN, African Union, Mercosur, etc. — to understand what drives regional cooperation globally. The discipline's late corrective to EU-centrism.

**Why this matters:** EU-derived theories (functionalism, neofunctionalism, liberal intergovernmentalism) struggle to explain ASEAN, which is consensus-based, sovereignty-preserving, and *conservative* rather than transformative — the opposite of the EU. Acharya's *constitutive localization* explains why: ASEAN borrowed the idea of multilateralism but reshaped it (the "ASEAN Way").

**Connected to:** → [[#Asian-origin global norms — ZOPFAN / nonintervention / human security / ASEAN regionalism|ASEAN regionalism]] (Week 4) — ASEAN is the canonical comparative regionalism case; → [[#constitutive localization + subsidiarity + deprovincialization|constitutive localization]] (Week 4) — explains how non-European regionalisms differ.

**Seen in:**
- [[Week 8 - Power Liberalism and World Order#^auto-power-liberalism-and-world-ord-16|Week 8 - Power Liberalism and World Order.md:L40]] — *"==**Comparative regionalism**=="*

---

## Week 9 — Global Governance and EU Regional Integration

| Term | One-liner |
|---|---|
| Liberal intergovernmentalism (Moravcsik) | EU integration = member-state bargains rooted in domestic economic interests |
| Subsidiarity (EU sense) | EU acts only where more effective than national level — NOT Acharya's sense |
| Normative power Europe (Manners) | EU shapes what counts as "normal" through norms and standards, not military force |

---

### Liberal intergovernmentalism — EU sense (Moravcsik)
*(Week 8 has a shorter entry on intergovernmentalism as an integration theory. This entry focuses on Moravcsik's specific three-step model.)*

**Definition:** Andrew Moravcsik (1993, 1998): EU integration is driven by bargains between major member states, whose national preferences are rooted in *domestic economic interests*. Three-step model:
1. **National preference formation** — domestic economic actors (export industries, financial sectors) shape what governments want from Europe.
2. **Interstate bargaining** — national positions clash and combine in Council negotiations; outcomes reflect relative bargaining power.
3. **Institutional choice** — states choose to *lock in* commitments through supranational institutions to make them *credible*. Institutions are servants of state preferences, not autonomous drivers.

**Moravcsik's empirical claim:** Major integration steps (Single European Act 1986, EMU, Amsterdam) were Franco-German *bargains*, driven by economic interests. EMU was driven by German exporters' interest in exchange-rate stability — not by technocratic spillover logic.

**Empty Chair Crisis (1965–66):** de Gaulle's France boycotted the Council for 7 months over moves toward qualified majority voting. Resolved by the *Luxembourg Compromise* — national veto preserved for "vital interests." Classic illustration of the intergovernmental logic overriding supranationalism.

**Connected to:** → [[#functionalism + neofunctionalism + spillover|functionalism + neofunctionalism + spillover]] (Week 8) — the rival theory: spillover drives integration vs. states drive it via bargains; → [[#comparative regionalism|comparative regionalism]] (Week 8) — LI struggles to explain ASEAN, which is sovereignty-preserving and consensus-based (the opposite of Moravcsik's bargaining logic).

**Seen in:**
- [Week 9 - Global Governance and EU Regional Integration.md:L23](projects/finals-y2-s2/IRO/Summaries/Week%209%20-%20Global%20Governance%20and%20EU%20Regional%20Integration.md) — *"==**Liberal intergovernmentalism (LI)**=="*

---

### Subsidiarity (EU sense)
*(Critical distinction — NOT the same as Acharya's subsidiarity from Week 4)*

**Definition (EU/Treaty sense):** Treaty principle (Article 5 TEU): the EU only acts where action at EU level is *more effective* than at national level. Decision-making should happen at the lowest feasible level of governance. If member states can handle something, the EU should step back.

**Why the Week 4 distinction matters:** Acharya's "subsidiarity" (Week 4) means something *entirely different* — local norms diffusing *outward* to global governance (human security spreading from Asia globally). The EU sense is *downward* (EU → national); Acharya's sense is *upward* (local → global). Same word, opposite direction. The exam will test whether you can keep these apart.

**Connected to:** → [[#constitutive localization + subsidiarity + deprovincialization|subsidiarity (Acharya sense)]] (Week 4) — same word, fundamentally different direction and meaning; → [[#Liberal intergovernmentalism — EU sense (Moravcsik)|liberal intergovernmentalism]] — Moravcsik's theory predicts subsidiarity will expand where national interests demand it; neofunctionalists predict spillover will gradually hollow it out.

**Seen in:**
- [Week 9 - Global Governance and EU Regional Integration.md:L27](projects/finals-y2-s2/IRO/Summaries/Week%209%20-%20Global%20Governance%20and%20EU%20Regional%20Integration.md) — *"==**Subsidiarity (EU sense)**=="*

---

### Normative power Europe (Manners)

**Definition:** Ian Manners (2002): the EU is a *normative* power — it shapes what is considered "normal" in international relations through norms, standards, and rules, rather than through military coercion or pure economic leverage. The EU exports its regulatory framework (GDPR, REACH, human rights conditionality), its legal standards, and its political norms (democracy, rule of law) to neighbours and global partners.

**The "Brussels effect":** The EU is so large that even non-EU firms comply with EU regulations to access the single market. The EU *de facto* sets global standards without coercive enforcement.

**The "soft imperialism" critique:** Critics argue the EU's promotion of "universal" norms is itself Eurocentric imposition — *whose* norms get called "normal"? Human rights conditionality can function as political leverage. This connects directly to the decentring/postcolonial critique (Week 5/Week 9).

**Connected to:** → [[#hard / soft / smart power (Nye)|hard/soft/smart power]] (Week 8) — normative power sits between soft power (attraction through culture) and structural power (embedding the rules); → [[#constructivism + "where do those interests come from?"|constructivism]] (Week 2) — normative power is constructivism's empirical prediction applied to the EU: if identity and norms matter, the EU's norm-spreading is genuine power; → [[#deprovincialization|deprovincialization]] (Week 4) — the "soft imperialism" critique applies the same question Acharya asks about Western IR theory to EU foreign policy.

**Seen in:**
- [Week 9 - Global Governance and EU Regional Integration.md:L32](projects/finals-y2-s2/IRO/Summaries/Week%209%20-%20Global%20Governance%20and%20EU%20Regional%20Integration.md) — *"==**Normative power Europe (Manners)**=="*

---

## Week 10 — Nation, Race, and Gender

| Term | One-liner |
|---|---|
| Imagined community (Anderson) | The nation as a horizontal community made thinkable by print capitalism |
| Civic nationalism + Ethnic nationalism | Membership via institutions vs via descent — Korea and Ukraine as test cases |
| Masculinist nationalism (Nagel) | The nation is also a project of gender-building — honour, sacrifice, defence |

---

### Imagined community (Anderson)
*(The modernist case for what a nation is)*

**Definition:** Benedict Anderson (1983): the nation is *"an imagined political community — imagined as both inherently limited and sovereign."* Members never meet most fellow members; communion lives in their minds. Created by *print capitalism*: as Latin lost ground, capitalist publishers needed mass audiences in vernacular languages. Novels and newspapers taught readers to imagine many strangers *simultaneously* moving through the same time — making the nation thinkable as a horizontal community.

**Four features:** (1) *Imagined* — communion lives in the mind; (2) *Limited* — finite, if elastic, boundaries; (3) *Sovereign* — replaces divine authority; (4) *Community* — conceived as horizontal comradeship despite internal inequality.

**Modernist argument:** Nations are *modern* (late 18th / 19th century), not ancient. Nationalism *creates* nations — not the other way around. The timing puzzle (why does nationalism appear when it does?) is answered by print capitalism + industrialisation: industrial economies need a single high culture → the state produces it through mass schooling (Gellner).

**Connected to:** → [[#Civic nationalism + Ethnic nationalism|civic vs. ethnic nationalism]] — Anderson's modernism explains both forms as historical constructions, not ancient essences; → [[#populism: demand vs supply + populism as repertoire vs nationalism|populism as repertoire vs nationalism as structure]] (Week 11) — Anderson's long-durée nationalism is exactly the "structure" that populism is a "style" layered on top of.

**Seen in:**
- [Week 10 - Nation Race and Gender.md:L24](projects/finals-y2-s2/IRO/Summaries/Week%2010%20-%20Nation%20Race%20and%20Gender.md) — *"==**Imagined community (Anderson)**=="*

---

### Civic nationalism + Ethnic nationalism
*(Two ideal types — with Korea and Ukraine as the test cases)*

**Civic nationalism:** Membership grounded in shared *political institutions*, laws, and commitments. Anyone who accepts the institutions can join. Identity is anchored in the state and its legal framework. Examples: France's revolutionary tradition; the US in self-image; post-2014 Ukraine (panel survey evidence: civic attachment rises under invasion; Russian-speaking Ukrainians identify as Ukrainian).

**Ethnic nationalism:** Membership grounded in *descent*, language, religion, shared history. Harder to acquire — you can't simply join by accepting institutions. Examples: 19th-century German romanticism; modern South Korea (*tanil minjok* — "shared blood"); post-colonial cases where the state was absent under colonialism.

**Key insight from Korea/Ukraine cases:** Whether self-preservation runs through ethnic or civic channels *depends on the institutional context*, not something fixed in "the people." When civic infrastructure exists (Ukraine: constitution, elections, institutions), civic does the work. When it doesn't (Korea under Japanese colonial rule: no state), ethnic fills the gap. The ethnic/civic distinction is *analytically useful but empirically fluid* — all nationalisms mix both; the question is which axis is doing the heavier work right now.

**Connected to:** → [[#Imagined community (Anderson)|imagined community]] — Anderson's modernism explains both as historical constructions; → [[#populism: demand vs supply + populism as repertoire vs nationalism|populism as repertoire vs nationalism as structure]] (Week 11) — civic nationalism is more compatible with liberal democracy, but both forms can be weaponised by populist movements.

**Seen in:**
- [Week 10 - Nation Race and Gender.md:L30](projects/finals-y2-s2/IRO/Summaries/Week%2010%20-%20Nation%20Race%20and%20Gender.md) — *"==**Civic nationalism**=="*
- [Week 10 - Nation Race and Gender.md:L31](projects/finals-y2-s2/IRO/Summaries/Week%2010%20-%20Nation%20Race%20and%20Gender.md) — *"==**Ethnic nationalism**=="*

---

### Masculinist nationalism (Nagel)

**Definition:** Joane Nagel (1998): *"Nationalism is a project of nation-building, but also a project of gender-building."* The nation is an *ideologically masculine* project — organised around honour, sacrifice, defence, and conquest. The classics of nationalism theory (Gellner, Anderson, Hobsbawm) are stories about men; this is not accidental but structural.

**Three gendered roles:**
- *Masculinist heroism:* men fight, rule, and are remembered. The soldier-citizen is the nationalist icon.
- *Patriotic motherhood:* women carry, raise, and embody the nation. Women's reproductive autonomy becomes a nationalist-political question.
- *Sexualized militarism:* enemy men framed as predatory and emasculated; enemy women as licentious and dishonourable. Defends the masculine/heterosexual character of military and political institutions.

**Why it matters for IR:** Feminist IR scholars have critiqued the field as embedded in a masculinist logic — "high politics" (war, diplomacy, security) is gendered male; "low politics" (welfare, health, migration) gendered female. The hierarchy of what counts as "real" IR is itself gender-constructed. Contemporary manifestations: immigrant men framed as threats to "our" women; women dual-positioned as victims and reproducers of the national community.

**Connected to:** → [[#decolonising war + small wars (Barkawi)|decolonising war]] (Week 7) — Barkawi's decolonial critique and Nagel's feminist critique are structural parallels: both expose how mainstream IR's categories were built to exclude certain phenomena and actors; → [[#Imagined community (Anderson)|imagined community]] — Anderson's analysis of the nation is gender-blind; Nagel's is the feminist supplement that shows the implicit gender architecture.

**Seen in:**
- [Week 10 - Nation Race and Gender.md:L38](projects/finals-y2-s2/IRO/Summaries/Week%2010%20-%20Nation%20Race%20and%20Gender.md) — *"==**Masculinist nationalism (Nagel)**=="*

---

## Week 11 — Globalization and Populism (Denney)

| Term | One-liner |
|---|---|
| globalization trilemma (Rodrik) | See Week 8 — same concept, deepened in Week 11 with the "deep = regulatory harmonization, not trade volume" nuance |
| globalization of rage (Mishra) | Globalization produced psychic dislocation — affective layer of populism |
| populism: demand vs supply (Rodrik) | Demand from globalization shocks; supply (left/right) depends on cleavage activated |
| populism as repertoire vs nationalism as structure | Populism is a style you pick up; nationalism is the structural condition |
| precariat (Standing) | Insecure, low-wage, fixed-term workers — the bloc fuelling populism |

---

### globalization of rage (Mishra)

**Definition:** Pankaj Mishra (2016, 2017): globalization produced not only inequality but **psychic dislocation**. Modernity promised dignity and did not deliver; the political affect is **resentment**, organized around identity and belonging.

**The long arc:** Today's reactionary movements have ancestors in 19th-century romantic nationalists, anarchists, revolutionaries. Same grievances, different content. Mishra's point: this is not a temporary pathology — it's the predictable emotional response to the broken promises of modernity.

**Why Mishra completes the diagnosis:** Rodrik's trilemma gives the *constraint*. Heywood & Whitham give the *conditions* (thinning of social ties). Mishra fills in the *affect* — why anger, not resignation, is the dominant response.

**Connected to:** → [[#globalization trilemma (Rodrik)|globalization trilemma (Rodrik)]] — Rodrik is structural-economic, Mishra is emotional-psychological; together they explain why populism is the political form; → [[#populism: demand vs supply + populism as repertoire vs nationalism|populism: demand vs supply]] — the rage is the demand side; cleavage activation is the supply side; → [[#precariat (Standing)|precariat]] — the social bloc carrying the rage.

**Seen in:**
- [[Week 11 - Globalization and Populism#^auto-globalization-and-populism-l32|Week 11 - Globalization and Populism.md:L32]] — *"==**Globalization of rage**=="*

---

### populism: demand vs supply + populism as repertoire vs nationalism
*(Two Rodrik/Denney distinctions that need to be kept together)*

**Demand vs. supply (Rodrik 2018):**
- **Demand** for populism: anti-establishment grievance generated by globalization shocks. *Universal* across affected countries.
- **Supply** (which cleavage gets activated): depends on the *type* of shock. Trade/finance shocks → class cleavage → **left populism** (Latin America: Chávez, Morales, Kirchner, Correa). Immigration shocks → ethno-cultural cleavage → **right populism** (Western Europe: Le Pen, Orbán, AfD, Meloni; also Trump, Brexit).

**Crucial pedagogical caveat** (Denney flagged this hard): *"Globalization explains right-wing populism"* is the **lazy reading** Rodrik resists. Globalization explains the *demand* for populism. It does *not* explain which form populism takes. Two countries hit by the same shock can produce opposite political responses.

**Populism as repertoire vs nationalism as structure:** Populism is a *political style* (people vs. elite) that can be picked up by parties of left or right — distinct from nationalism. **Populism is portable; nationalism is structural.** Same point in metaphor: nationalism is a long-standing building you live in; populism is a piece of music you can play inside that building. Korea and Japan have strong nationalisms but very little durable populism — the building is there but the music isn't being played.

**The US case** is the clearest illustration of demand/supply: same country, same period, two populisms (Sanders left + Trump right) along different cleavages of the same crisis.

**Connected to:** → [[#globalization trilemma (Rodrik)|globalization trilemma]] — the structural cause of the demand; → [[#globalization of rage (Mishra)|globalization of rage (Mishra)]] — the affective register of the demand; → [[#precariat (Standing)|precariat]] — the social bloc whose votes carry the populist surge.

**Seen in:**
- [[Week 11 - Globalization and Populism#^auto-globalization-and-populism-l11|Week 11 - Globalization and Populism.md:L11]] — *"==**Dani Rodrik**== ... for the globalization trilemma and the demand/supply distinction"*

---

### precariat (Standing)

**Definition:** Guy Standing (2011): the new class of workers on **insecure, low-wage, fixed-term labor** without the protections their parents' generation had. A sizable fraction of the under-40 workforce in rich democracies — and they vote differently.

**Why it matters:** The precariat is the social-class substrate of the populist surge. Standing's diagnosis fits between Heywood & Whitham (thinning of social ties → the conditions) and Rodrik (trilemma → the constraint): the precariat is the *empirical group* on which the constraint visibly bites and the conditions visibly hold.

**Connected to:** → [[#globalization trilemma (Rodrik)|globalization trilemma]] — the precariat bears the cost of whichever corner gets sacrificed; → [[#globalization of rage (Mishra)|globalization of rage]] — the precariat is the demographic carrier of Mishra's resentment; → [[#populism: demand vs supply + populism as repertoire vs nationalism|populism: demand vs supply]] — the precariat's vote is what gives populism its electoral weight.

**Seen in:**
- [[Week 8 - Power Liberalism and World Order#^auto-power-liberalism-and-world-ord-17|Week 8 - Power Liberalism and World Order.md:L247]] — *"==This produced the "precariat"=="*

---

## Week 12 — Anthropocene and Global Futures (Denney)

| Term | One-liner |
|---|---|
| Anthropocene | Geological epoch dominated by human activity; planetary scale exceeds state action |
| post-humanism | Decenters human as sole subject of ethics; takes nonhumans seriously as actors |
| Audra Mitchell — "Is IR going extinct?" | IR's anthropocentric, state-centric ontology can't grasp planetary loss |

---

### Anthropocene + post-humanism
*(Mitchell — these form a single ontological argument)*

**Anthropocene:** A proposed geological epoch in which human activity is the dominant force shaping earth systems. In IR, the term flags planetary processes (climate, biodiversity collapse, ocean systems) that **exceed the scale of state action**.

**Post-humanism:** A family of approaches that decenters the human as the sole subject of ethics and politics; takes nonhuman and inhuman forces as actors in their own right. **Mitchell's proposed alternative to IR's anthropocentric default.**

**How they connect — the ontological argument:** Mitchell's claim is that the gap between IR and planetary problems is **ontological, not just empirical**. IR's basic unit (the state) is bounded and territorial; planetary processes are border-crossing and operate on geological timescales. The standard response — *better management* through better treaties, better data, better targets — treats extinction as a problem of biopolitical management. But the gap is deeper: the field cannot yet **think the loss of life forms *as such*** because its starting premise (humanity as the only subject) rules the question out of bounds. Post-humanism is the proposed alternative ontology that makes the question thinkable: decentre the human, take nonhumans seriously, extend the temporal horizon, root politics in a shared planet (*cosmopolitics*).

**Explained easily:** Imagine you go to your doctor about a pain in your leg, and the doctor only owns measuring tools for the upper body. The doctor will keep measuring your arms and chest and finding nothing wrong, because the tools were built for a different part of the body. Mitchell says IR is like that doctor — its tools (state, sovereignty, anarchy) were built for one kind of problem (war and order between states). Planetary extinction is a different kind of problem entirely, and the tools don't reach it. You can't fix it by measuring more carefully with the wrong instrument.

**Connected to:** → [[#decolonising war + small wars (Barkawi)|decolonising war (Barkawi)]] (Week 7) and [[#NWIRT + Global IR|NWIRT + Global IR]] (Week 4) — same structural critique: the discipline's foundational concepts are not universal and rule certain phenomena out of view; → [[#globalization trilemma (Rodrik)|globalization trilemma]] (Week 8/11) — Denney noted in Week 12 that the trilemma's third squeezed corner becomes *planetary boundaries* once you take the Anthropocene seriously.

**Seen in:**
- [[Week 12 - Anthropocene and Global Futures#^auto-anthropocene-and-global-future|Week 12 - Anthropocene and Global Futures.md:L21]] — *"==**Anthropocene**=="*
- [[Week 12 - Anthropocene and Global Futures#^auto-anthropocene-and-global-future-2|Week 12 - Anthropocene and Global Futures.md:L26]] — *"==**Post-humanism**=="*

---

## ⚠️ Unclosed annotations — answer these or fix the syntax

These lines in your notes used `==` to start a question/comment but the closing `==` is missing. The scanner skipped them. Close them with `==` at the end and re-run `/extract-highlights`, or just ask me directly.

**Line 447** (Week 6) — Question about Paris School and securitisation:
> *"is the paris school also under securitization of migration, just another approach of obtaining it or what is the relation?"*

**Answer:** Yes — the Paris School is a *variant* of securitisation thinking, not a separate theory. The Copenhagen School (Wæver) says securitisation happens through **speech acts**: a politician labels migration a security threat and that declaration creates the security frame. The Paris School (Bigo) says securitisation happens through **everyday practices** of bureaucratic professionals — border guards, customs officers, database engineers — who build a surveillance infrastructure through routine work. Huysmans synthesises both. For your exam: name Copenhagen for "discourse and speech acts," Paris/Bigo for "bureaucratic practice and the internal security field," note they're complementary, and Huysmans draws on both.

---

**Line 471** (Week 6) — Question about the Germany / liberal paradox example:
> *"Example to the liberal paradox is germany that was inviting turkish refugees to work until 1973 when they wanted to stop but not really successfully. Then with the citizenship reform 1999 jus soli and 2000 green card, they wanted only tech workers but again found out that they cant have just the workers and not their families, children etc - is this right?"*

**Answer:** Yes, that's essentially correct. Germany recruited Turkish *Gastarbeiter* (guest workers) from 1955 under a bilateral agreement. After the 1973 oil shock Germany suspended recruitment — but courts upheld family reunification rights, so the population stayed and grew. The 1999 *jus soli* reform gave citizenship by birth on German soil (a first), and the 2000 Green Card tried to attract only IT specialists — but ran into the same wall: once workers settle, families arrive, children are born German, and you can't treat them as temporary. That's the paradox in action: each attempt to manage migration cleanly (only workers, only skilled, only temporary) gets overridden by the legal rights liberal courts protect. Small correction: they were recruited as *workers*, not refugees — but the dynamic you described is exactly right.

---

## One-Page Revision Sheets

*Cheat sheets for each lecture, pulled from the corresponding summary file.*

### Week 1 — Old Debates in a New World

**Frame:** Three post-Cold War visions of where world politics is heading — Fukuyama (liberal endpoint), Huntington (civilizational clash), Kagan (return of great-power competition). All three are tested against the empirical reality of 2014–2026: autocratisation wave + great-power rivalry + weaponised interdependence.

**The trends to know (2025–26):**
- **Autocratisation** (V-Dem 2025): democracy level back to 1985; 91 autocracies vs 88 democracies; liberal democracy = least common regime; 72% of humans under autocracy.
- **End of arms control**: New START winding down.
- **Weaponised interdependence**: tariffs, sanctions, supply-chain coercion (Carney Davos 2026: "a rupture, not a transition").

**The three thinkers compared:**

| | **Fukuyama** | **Huntington** | **Kagan** |
|---|---|---|---|
| Driver | Ideology / political systems | Culture / civilization | Power, geopolitical rivalry |
| World image | Convergence → liberal democracy | Fragmentation into civilizations | Return of great-power competition |
| IR affinity | Liberalism / Hegelian | Constructivism (culturalist) + realist undertones | Realism / neoconservatism |
| Main critique | Underestimates nationalism, religion, power | Essentializes cultures; intra-civilizational conflict refutes thesis | Normalizes militarism; underplays norms |

**Huntington's 7–8 civilizations:** Western · Confucian · Japanese · Islamic · Hindu · Slavic-Orthodox · Latin American · ('possibly') African.

**Huntington's problems (lecture critique):**
1. Civilizations don't inevitably clash (India–US ties).
2. Too many intra-civilizational conflicts (Sunni–Shia, Russia–Ukraine).
3. Essentializes cultures as monolithic.
4. Too simplistic for a fragmented, multipolar world.

**Kagan's key tension** (textbook p.342): *"Tensions between democratic states (US–EU) and between authoritarian states (China–Russia) may be just as significant as those across the democracy/authoritarian divide."*

==**Acharya's multiplex world order**== — the contemporary critique of all three: future world order will not be dominated by ONE nation or civilization; *cultural and political diversity is the organising principle*. "Multiple shows on offer, giving the audience a choice of producers, directors, actors and plots."

**IR vs International Studies:** IR = states + power + war (poli-sci/econ roots). International Studies = global issues + culture + identity (interdisciplinary, critical, postcolonial).

**Lines to memorise:**
> *"The end of history as such: the end point of mankind's ideological evolution and the universalization of Western liberal democracy as the final form of human government."* — Fukuyama (1989)

> *"The principal conflicts of global politics will occur between nations and groups of different civilisations."* — Huntington (1993)

> *"This bargain no longer works. We are in the midst of a rupture, not a transition."* — Carney (Davos 2026)

---

### Week 2 — Power, Institutions and Ideas (Traditional IR Theories)

**Frame:** Three traditional schools — realism, liberalism, constructivism — and how they read the same events (Ukraine-Russia, US-China, NATO) differently. All three accept anarchy as the starting point; they disagree on what follows from it.

**The trio compared:**

| | **Realism** | **Liberalism** | **Constructivism** |
|---|---|---|---|
| Roots | Thucydides, Machiavelli, Hobbes | Locke, Kant, Smith | Post-positivist |
| Anarchy | Inherent; leads to conflict | Mitigated by institutions | Socially constructed |
| Interests | Fixed: power + security | Fixed: cooperation possible | Shaped by identity |
| Actors | States | States + institutions | States + norms |
| Epistemology | Positivist | Positivist | Post-positivist |

**Realism sub-variants:**
- **Classical (Morgenthau, Carr):** pessimistic HUMAN NATURE.
- **Neorealism (Waltz):** STRUCTURE of anarchy, not human nature.
- **Offensive realism (Mearsheimer):** great powers MAXIMIZE power → regional hegemony.
- **Defensive realism (Waltz, Walt):** SECURITY maximisers, prefer balancing + restraint.
- **Neoclassical (Rose, Schweller):** structure conditioned by DOMESTIC factors.

**Liberalism variants:**
- **Interdependence liberalism:** economic exchange raises conflict cost.
- **Republican (democratic) liberalism:** democracies don't fight each other (Kant, Doyle).
- **Neoliberal institutionalism:** institutions reduce uncertainty + transaction costs (Keohane, Nye).
- ==**Liberal International Order (LIO):**== combines all three; depends on legitimacy + restraint, not force alone (Ikenberry, Deudney).

==**Wendt's three cultures of anarchy:**==
- **Hobbesian** = enemies (early 20th c. Europe).
- **Lockean** = rivals respecting sovereignty (Cold War).
- **Kantian** = partners with shared norms (EU/NATO).

==**Mearsheimer's "Bound to Fail" (2019):**== LIO required liberal hegemony → US enforcement → backlash + costly interventions → cannot survive return of multipolar great-power competition. Realist critique of liberalism.

**Responses to Mearsheimer:**
- Liberal: institutions are resilient; problems are implementation, not design.
- Constructivist: "failure" is itself a narrative; norms are deeply embedded.

**Applying the trio to Ukraine-Russia:**
- *Realist:* security dilemma (NATO expansion → Russian backlash); Ukraine as strategic buffer.
- *Constructivist:* identity (imperial narrative; Russian / Ukrainian self-image).
- *Liberal:* institutional failure (UN veto) but sanctions = interdependence-as-leverage.

**What each theory explains well — and badly:**

| Theory | Strong | Weak |
|---|---|---|
| Realism | Power competition, security dilemmas, alliance formation | Cooperation, norm change, democratic peace |
| Liberalism | Institutional cooperation, trade peace, democratic peace | Why institutions fail, why orders collapse |
| Constructivism | Identity politics, norm diffusion, perception | Predicting specific outcomes, material constraints |

**Lines to memorise:**
> *"The social world is but a projection of human nature onto the collective plane."* — Morgenthau (classical realism)

> *"Anarchy is what states make of it."* — Wendt (1992)

> *"Theory is always for someone and for some purpose."* — Cox (Week 5, but applies here too)

---

### Week 3 — Nuclear Proliferation

**Frame:** Six moves — IR review → security dilemma → nuclear landscape → Sagan's three models → Waltz-Sagan debate → arms control unravelling.

==**Security dilemma:**== one state's efforts to increase its security make others *less* secure → spiral. Defensive measures perceived as offensive. Arms races emerge even when no one wants conflict. Applied to nukes: "proliferation begets proliferation."

**Nuclear landscape today:**
- **9 nuclear states:** NPT-recognised — {US, Russia, UK, France, China}; outside NPT — {India, Pakistan, Israel}; withdrew + acquired — {North Korea (2006)}.
- **Arsenals:** peaked ~70,000 (late 1980s) → ~12,500 today. Russia ~6,375 + US ~5,800 dominate; China ~320 expanding rapidly.
- **Gave up:** South Africa, Ukraine, Kazakhstan, Belarus, Libya.

**Key nuclear concepts:** deterrence · extended deterrence (nuclear umbrella) · MAD · first-strike vs second-strike · nuclear taboo · stability-instability paradox.

**Three perspectives on proliferation (mapping IR traditions):**

| Tradition | Logic | Policy |
|---|---|---|
| Realist | Survival in anarchy; security drives proliferation | Balance of power, deterrence |
| Liberal | Institutions can constrain proliferation | NPT, IAEA, CTBT |
| Constructivist | Norms + identity + nuclear taboo | Norm-building, stigmatization |

==**Sagan's three models (1996):**==

| Model | Logic | Lecturer's case |
|---|---|---|
| **Security** | External threats → bomb to deter | USSR 1949 (Stalin: "Provide the bomb — it will remove a great danger from us") |
| **Domestic politics** | Bureaucracies + politicians push for parochial gains | India 1974 (waited 10 years after China 1964 — security alone can't explain timing; Gandhi's domestic position can) |
| **Norms** | Bomb as symbol of modernity / prestige / status | France 1960 (de Gaulle: "A France without world responsibility would be unworthy of herself") |

Sagan's bottom line: **"multicausality, rather than measurement error, lies at the heart of the nuclear proliferation problem"** (p.85).

**Beyond Sagan:**
- **Hymans (2006):** leader psychology + oppositional nationalism.
- **Solingen (2007):** outward-oriented economies less likely to proliferate (have more to lose from sanctions).
- **Kartchner & Larsen (2009):** strategic culture.
- **Leverage / bargaining:** the program itself as diplomatic tool (North Korea).

==**Waltz-Sagan proliferation debate:**==
- **"More may be better" (Waltz):** rational states under deterrence avoid war → spreading nukes extends caution. Evidence: no nuclear war since 1945; Cold War "long peace"; India-Pakistan avoided direct war.
- **"More will be worse" (Sagan):** organisations have pathologies — preventive war, accidents (Petrov 1983, Goldsboro 1961), civil-military dysfunction, irresponsible powers. More nukes = higher catastrophic-failure risk.
- Mainstream IR has tilted toward Sagan since early 2000s.

**Stability-instability paradox:** nuclear stability at strategic level may *enable* conventional conflict at lower levels (India-Pakistan limited wars under nuclear umbrella).

**Arms control regime + key dates:**
- **NPT (1968)** — three pillars (non-proliferation, disarmament, peaceful use); 191 parties; cornerstone treaty.
- **IAEA** — verification + safeguards.
- **CTBT (1996)** — NOT in force.
- **NWFZs** — Latin America, SE Asia, Africa, Central Asia, South Pacific.
- Timeline: LTBT 1963 · NPT 1968 · SALT I / ABM 1972 · INF 1987 · START I 1991 · CTBT 1996 · New START 2010 · TPNW 2017.

==**The unravelling (as of Feb 2026):**==
- 2019: US withdraws from INF.
- 2023: Russia suspends New START participation.
- ==**5 February 2026: New START expires without renewal — the first time in 50+ years with NO binding limits on US/Russian nuclear arsenals.**==
- JCPOA collapsed 2018; North Korea ICBMs reach continental US; China expanding fast.

**Lines to memorise:**
> *"Nuclear weapons ... are more than tools of national security; they are political objects ... and can also serve as international normative symbols of modernity and identity."* — Sagan (1996, p.55)

> *"Multicausality, rather than measurement error, lies at the heart of the nuclear proliferation problem."* — Sagan (1996, p.85)

> *"Proliferation begets proliferation."* — Sagan (1996, p.57)

---

### Week 4 — Non-Western IR Theory

**Frame:** IR is "an American social science" (Hoffmann 1977). Built from Westphalia. Universalises one region's history. **Hegemonic in the Gramscian sense.**

**Why it matters:** Different historical international systems existed — **tributary** (Sinic, hierarchical), **Mandala** (Southeast Asian, overlapping). Western IR misses these and treats them as deviations.

**The Asia debate:**
- Friedberg: ripe for rivalry (neorealism).
- Kang: bandwagoning + hierarchy (culturalist realism).
- ==Acharya: **double-binding + engagement + norms + institutions**== (NWIRT). Asia is more stable than predicted because of economic interdependence, Bandung-era norms (non-interference, sovereignty), and ASEAN-led institutional architecture.

**Three pathways forward:**
1. Extend Western IRT (insufficient).
2. Asian exceptionalism (dangerous — "Asian values," Tianxia romanticism).
3. **Global IR / deprovincialization** (Acharya's choice).

==**Two mechanisms of Global IR:**==
- ==**Constitutive localization** — foreign ideas blend into local norms (e.g. ASEAN Way).==
- ==**Subsidiarity** (Acharya) — local norms diffuse outward (e.g. nonalignment, human security).==

**Korea case ("twisted postcoloniality"):** colonised by Japan, then US-dependent. Two trap strategies: universalist (mimic) or particularist (marginal). Proof that structural barriers — not lack of ideas — keep non-Western IR underdeveloped.

**Survey punchline:** Same scenario, different historical analogy → different conclusions. *The history you start from shapes the theory you build.*

**The line to memorise:**
> *"Global IR is not about rejecting the Western IR tradition, but broadening the scope and sources of IR to make it more inclusive and universal."* (Acharya 2014)



%%Love this one page revision btw!! great work, do this for all of the summaries you make - all important on one page, key terms, easy explanations, key quotes, love it!%%

---

### Week 5 — Critical and Decentring Perspectives

**The meta-distinction (memorise):**
- **Problem-solving theory** (Cox 1981): takes the world as given; helps the powerful manage it. Mainstream IR.
- **Critical theory**: asks how the world came to be this way, whose interests it serves, how it might be changed. Emancipatory, post-positivist.

**The five major critical lineages:**
1. **Marxism / world-systems** — capitalism as global structure; core/periphery/semi-periphery; uneven development.
2. **Frankfurt School / Gramsci** — power as consent + coercion; hegemony of bourgeois ideas as "common sense"; counter-hegemony.
3. **Feminism** — IR is gender-blind; "power politics" is built on masculinist assumptions; standpoint theory; intersectionality (Crenshaw).
4. **Poststructuralism / Foucault** — discourse and power-knowledge; biopolitics; "there is nothing outside the text" (Derrida); deconstruction.
5. **Postcolonialism** — Orientalism (Said); provincialise Europe (Chakrabarty / Seth); ubiquity ≠ universality; colonial encounters constitute the modern international order.

**Seth's three-part critique:**
- **History** — the "expansion of international society" narrative sanitises slavery, conquest, genocide. Postcolonial accounts treat colonialism as **constitutive**, not a footnote.
- **Culture/theory** — the "procedural" universality of international law is never neutral. *Ubiquity is not universality.*
- **Knowing/being** — knowledges **constitute** what they purport to represent. IR naturalises what is historically produced. The "free, rational individual," the nation-state, sovereignty — all are products of historical processes and discourses, not discoveries.

**Decentring (Fisher-Onar & Nicolaïdis 2013):** provincialise → engage → reconstruct.

**Hoffmann's line (the link to Week 4):** *"IR is an American Social Science."* (1977)

**The quote to memorise — from Seth:**
> *"A great deal of IR theory is an obstacle to a recognition and exploration of this, rather than a guide to it, for it seems content to naturalise what it could problematise, and to assume that which it should deconstruct."*

**The Cox line:**
> *"Theory is always for someone and for some purpose."* (Cox 1981)

---

### Week 6 — Global Migration: Mobility and Security

**Definitions to nail (1951 Convention is exam gold):**
- **Refugee** ==fear of persecution on race/religion/nationality/social group/political opinion== + outside country of nationality.
- **Asylum-seeker** = applied, case pending.
- **IDP** = displaced, hasn't crossed an international border.
- **Non-refoulement** = no return to country of persecution.

==**Hollifield in one minute==:**
- ==Liberal paradox: economic openness × political closure.==
- ==Three state functions: garrison + trading + **migration state**.==
- ==Rights are key: courts protect migrants → restrictive policies repeatedly struck down.==
- ==Guest worker schemes don't work — *"We asked for workers, but human beings came"* (Frisch).==
- ==EU = regional regime that lets states **finesse** (not escape) the paradox.==

**Huysmans in one minute:**
- Migration is **securitised** through political construction — speech acts + practices.
- The EU ==**internal market spillover into internal security**==: SEA → Schengen → Third Pillar → Dublin → Amsterdam.
- The **security continuum**: borders + terrorism + crime + migration get treated together.
- ==Three themes of securitisation: **internal security, cultural security, welfare chauvinism**.==
- Securitisation is **structural** — not just a few extreme actors.

**Schools of securitisation:**
- **Copenhagen** (Wæver, Buzan) — security = speech act.
- **Paris** (Bigo) — security = everyday bureaucratic practice; border guards are "managers of flows."

**Key institutions / agreements:**
- ==**1951 Geneva Convention** — refugee definition + non-refoulement.==
- **Schengen (1985)** — no internal border checks + common external visa policy.
- **Dublin (1990)** — first safe country rule.
- **Eurodac** — fingerprint database of asylum seekers.
- ==**Third Pillar (TEU 1992)** — JHA cooperation.==
- ==**Amsterdam (1997)** — migration moved from Third to First Pillar.==
- ==**Tampere (1999)** — Schengen acquis condition for accession.==
- ==**Frontex** — EU border agency (lecture mention).==
- **UNHCR / IOM / ILO** — postwar international migration architecture.

**Migration diplomacy:**
- Coercive (blackmail) vs. cooperative (backscratching).
- Externalization = pushing border control to third countries.
- EU-Tunisia 2023: €105m coastguards + €900m macro support + €150m budget support.
- EU-Turkey 2016 deal is the prototype.
- Material gains (economic, institutional, aid) + symbolic gains (status, reputation, moral standing).

**The lines to remember:**
> *"Why do liberal states accept unwanted immigration?"* (Joppke 1998)

> *"We asked for workers, but human beings came."* (Max Frisch on European guest workers)

> *"It is by labelling something a security issue that it becomes one."* (Wæver 2004)

> *"Migration is a nodal point in the internal security field."* (Huysmans 2000)

---

### Week 9 — Global Governance and EU Regional Integration

**Frame:** The EU is *more than an international organisation, less than a state*. The lecture explains what kind of region it is, how it integrates, and what kind of global power it exerts.

**Region & regionalism:**
- Region = geography + interaction + identity (Cantori & Spiegel; Katzenstein).
- Explosion of regional organisations since the 1990s. Comparative regionalism: don't treat the EU as the universal template.

**EU institutional triangle:**
- **Commission** — initiative + executive (supranational).
- **Parliament** — co-decision + control (directly elected).
- **Council** — member states, decision-making (QMV or unanimity).

**Four competences:** exclusive (trade, internal market) · shared (environment, transport) · complementary (education) · intergovernmental (CFSP). On the basis of *subsidiarity* + *proportionality*.

**Theories of integration:**
| Theory | Driver | Key thinker |
|---|---|---|
| **Neofunctionalism** | Spillover (functional + political) — sector cooperation drags in politics | Ernst Haas |
| ==**Liberal intergovernmentalism**== | Member-state bargains rooted in domestic economic interests; 3-step model (preferences → bargaining → credible commitments) | Andrew Moravcsik |
| Intergovernmentalism | The nation-state is "obstinate" | Stanley Hoffmann |
| EU as nation-state rescue | EU *strengthens* the nation-state | Alan Milward |
| Crisis-driven Europeanisation | Covid-19 forged coordinative reflexes | Wolff & Ladi (2021) |

**Empty Chair Crisis (1965–66):** de Gaulle's 7-month Council boycott over QMV. Resolved by *Luxembourg Compromise* — national veto preserved for "vital interests." Classic supranational-vs-intergovernmental test.

**What kind of global power is the EU?**
- Civilian (Duchêne) · soft (Nye) · hard? (limited) · ==**normative (Manners)**== · ethical (Aggestam) · regulatory · soft imperialism (critics).
- Manners: EU shapes what is *normal* via norms, standards, rules — not arms.
- Structural problem: external action fragmented across exclusive/shared/intergovernmental competences.

**Zwolski 2025 — Pan-Europe revisited:**
- Today's EU "geopolitical power" rhetoric (ReArm Europe, strategic autonomy) echoes *inter-war* pan-European debates.
- Coudenhove-Kalergi (1923, *Pan-Europa*): unite Europe to resist American + Soviet hegemony.
- Lineage matters: the geopolitical EU is not new; it revives what the post-1945 civilian-power EU was specifically designed to avoid.

**Decentring EU studies (Nicolaïdis):** mutual engagement + self-reflexivity + radical decentring (centring those speaking from elsewhere).

**Lines to memorise:**
> *"The political and the national unit should be congruent."* — Gellner (relevant to the EU's legitimacy gap)

> *"Europe is uniting without the majority of Europeans being ready for it."* (lecture opener — captures the pan-European legitimacy puzzle)

---

### Week 10 — Nation, Race and Gender

**Frame:** Nations are *imagined* but consequential. Where they come from (ancient vs modern), how they cohere (ethnic vs civic), and how race + gender structure them — that's the lecture.

**What is a nation? — three dimensions (Heywood ch.8):**
- **Cultural** (language, religion, history) · **Political** (claim to sovereignty) · **Psychological** (loyalty, patriotism).
- Nation ≠ state ≠ country ≠ ethnic group ≠ nation-state. Be precise.

==**Imagined community (Anderson 1983):**==
- Imagined (you'll never meet most members) · limited · sovereign · community (horizontal comradeship).
- Made thinkable by *print capitalism* — vernacular novels + newspapers → simultaneity.

**Primordialist vs Modernist:**

| | Primordialist | Modernist |
|---|---|---|
| Origin of nation | Ancient ethnic / cultural community | Late-18th / 19th-century political construction |
| Mechanism | Inherited bonds | Industry + print + mass schooling |
| Key thinkers | Herder, Smith, Connor | **Gellner, Hobsbawm, Anderson** |
| How nationalism relates | Nations come first; nationalism awakens them | Nationalism comes first; nations are its product |

**Gellner:** industrial economies require cultural homogeneity → state produces it via mass schooling → nationalism = "political and national unit should be congruent."

**Nationalism in IR theory:**

| Lens | What it sees |
|---|---|
| Realist | Fuel for state power (war, taxation, territory) |
| Liberal | Civic nationalism → self-determination + democratic citizenship (Wilsonian) |
| Constructivist | Identity as socially constructed, contested |
| Marxist | Ideology binding workers + capitalists (Anderson partly here) |

==**Civic vs ethnic nationalism:**==
- *Civic:* membership via institutions (France, US in self-image, Ukraine since 2014).
- *Ethnic:* membership via descent / language / religion (Germany 19th c., South Korea).
- *Ideal types* — all nationalisms mix both. The empirical question: which axis is doing the work *now*?

**Wilsonian vs reactionary nationalism:** same vocabulary (nation, sovereignty, the people), opposite projects — self-determination vs recovered glory + foreigner-as-threat.

**Korea + Ukraine — the take-home:**
- *Korea 1910–45:* Japanese rule abolished the state → only blood + language + dynastic memory survived → ethnic nationalism (*tanil minjok*).
- *Ukraine 2014–:* working civic institutions exist → civic attachment rises even under invasion; Russian-speakers identify *as Ukrainians*.
- ==**Lesson: ethnic vs civic depends on institutional context, not something fixed in "the people."**==

**Race, identity, othering (Heywood ch.9):**
- Identity is two-way: also *identification by* others (racialization).
- Nations are defined as much by who is OUT as IN — the boundary work is constitutive.
- *Hindutva* = live case: Hindu = Indian; Muslims excluded by law + violence.

==**Gender and nationalism (Nagel 1998):**==
- *"Nationalism is a project of nation-building, but also a project of gender-building."*
- The nation itself is an ideologically masculine project — honor, sacrifice, defense, conquest.
- Three gendered roles: *masculinist heroism* (men do war/rule) · *patriotic motherhood* (women embody / reproduce the nation) · *sexualized militarism* (enemy men predatory, enemy women licentious).
- Today: immigrant men framed as threats to "our" women; women dual-positioned (victims + reproducers).

**Nationalism today — resurgence:**
- Brexit · Trump · AfD · Le Pen · Orbán · Meloni · Modi · Putin.
- Shared: anti-globalist + exclusionary + recovered-glory + anti-IO.
- Different flavours: religious (India), economic (Trump), civilizational (Orbán, Putin), revanchist (Russia).

**Bridge to Week 11:** Nationalism today is a 21st-century *response* to the post-1989 globalization order, not a 19th-century holdover.

**Lines to memorise:**
> *"An imagined political community — imagined as both inherently limited and sovereign."* — Anderson

> *"The political and the national unit should be congruent."* — Gellner

> *"Nationalism is a project of nation-building, but also a project of gender-building."* — Nagel

---

### Week 7 — War, Peace and International Security

**Frame:** War is not a natural fact but a contested concept. The lecture moves from definitions through IR theories to Barkawi's decolonial critique.

**Definition trouble:**
- UN "major conflict" = ≥1,000 deaths/year — arbitrary; excludes the Falklands.
- Types: Blitzkrieg (speed), Total War (society mobilised), Hegemonic, Guerrilla, Revolutionary.

**IR theory takes on war:**
- **Realism (Clausewitz):** *"War is the continuation of political intercourse by other means."* War = power. Peace and war are NOT opposites.
- **Liberalism:** war is solvable — IOs, free trade, democracy (Deudney & Ikenberry 1999). Democratic peace theory.
- **Idealism:** perpetual peace (Kant); collective security; security communities (Deutsch 1957); Just War.
- **Postcolonialism:** world is hierarchical, not anarchic. Brings race, gender, class.

**Just War Theory:**
- **Jus ad bellum** (6): last resort · just cause · legitimate authority · right intention · reasonable prospect · proportionality.
- **Jus in bello** (3): discrimination · proportionality · humanity.

**Barkawi — decolonising war (3 Eurocentric building blocks to attack):**
1. War/peace are opposites → replace with battle/repression schema where force is *ordinary* politics.
2. War is between sovereign states → situate in transnational/imperial hierarchies.
3. War is international or civil → colonial wars don't fit either box.

==Insight: "Small Wars" (colonial uprisings) get dismissed as not-war. The Eurocentric definition is the problem, not the geographic focus.==

**New wars:**
- Kilcullen 2020: US 1991 military model no longer working.
- "Snakes & Dragons": non-state actors + Russia/China refusing US orbit. China's "three warfares" — psychological, public opinion, lawfare.
- PMSI: 1 in 4 US personnel in Iraq/Afghanistan was a private contractor (2016). Montreux Document 2008.
- War on Terror became "a threat without enemies."

**Re-Arm Europe (2025):** €800B plan, SAFE lending instrument, response to Ukraine.

**Lines to memorise:**
> *"War is simply a continuation of political intercourse, with the addition of other means."* — Clausewitz

> *"America is not at war. The Marine Corps is at war. America is at the mall."* — quoted in Barkawi

---

### Week 8 — Power, Liberalism and World Order

**Frame:** World order = stable patterns of relations among actors, built on power not justice. The post-1991 liberal order is in crisis.

**World order types:**
- Bipolar (Cold War: US vs USSR)
- Unipolar (1991 → mid-2000s: "unipolar moment" — Krauthammer)
- Multipolar (today: US, China, Russia, India...)
- ==**Multiplex (Acharya):** multiple shows, multiple producers — diversity is the principle, not just more powers.==

**Power — what is it?**
- Dahl: "ability to get others to do what they would not otherwise do."
- **Resource vs. relational:** what you possess vs. what you can do with it.
- **Hard / soft / smart power (Nye):** military+economic / culture+norms / both combined.
- **Institutional power:** embedded in rules (North, Thelen). The US shaped UN/WTO/IMF in its image.

**Rodrik's globalization trilemma:** deep globalization + nation-state + democracy — pick any two.
- EU = globalization + democracy (sacrificed sovereignty)
- China = globalization + nation-state (democracy never on offer)
- Brexit / Trump = nation-state + democracy (sacrificed depth)

**Crisis of liberal order:**
- 9/11 + neoconservatism → US unilateral primacy. Kagan: *"America did not change on 9/11. It only became more itself."*
- Acharya's multiplex: future order won't be Western OR Chinese.

**China's evolution:**
- Deng (1980s): *"bide time, hide brightness."*
- Hu: *"peaceful rise / peaceful development."*
- Xi (2010s–): *"Chinese Dream / Major Country Diplomacy."*
- **BRI (2013):** bilateral infrastructure deals; "debt trap diplomacy" critique.
- China & Global South: Bandung 1955 → Non-Aligned Movement → "champion" of developing world (asymmetric — China = 66% of GDP growth from emerging/developing 2000–2019).

**Line to memorise:**
> *"Future world order will not be dominated by one nation or civilization. What matters is cultural and political diversity."* — Acharya

---

### Week 11 — Globalization and Populism

**Frame:** Three stacked layers explain why globalization produced populism.

**The three-layer diagnosis:**

| Author | Layer | Core claim |
|---|---|---|
| Heywood & Whitham | Sociological | Social ties thinned — community → networks |
| Rodrik | Political-economic | Trilemma: globalization + nation-state + democracy — pick 2 |
| Mishra | Psychological | Modernity promised dignity, didn't deliver — rage |

**Layer 1 — society thinning:**
- **Precariat** (Standing) — insecure / low-wage workers without industrial-era protections.
- **Risk society** (Beck) — politics organised around *bads* (climate, pandemics).
- **Liquid modernity** (Bauman) — family / class / profession dissolved.
- **Network society** (Castells) — grievance moves faster than nuance.

**Layer 2 — Rodrik's trilemma (5-country diagnosis):**

| Country | Corner | Sacrifice |
|---|---|---|
| Greece (Eurozone) | Globalization + sovereignty (formal) | Substantive democracy (2015 referendum) |
| Brexit UK | Sovereignty + democracy | Depth of globalization |
| China | Globalization + nation-state | Democracy never on offer |
| US Trump | Sovereignty + democracy | Depth rolled back |
| Argentina Milei | Cycles between corners | Trilemma as treadmill, not fate |

- ==**Deep globalization = regulatory harmonization**, NOT trade volume. Brexit Britain still trades.==
- **Stolper–Samuelson:** trade makes some factors *absolutely* worse off.
- **Compensation problem:** distortionary + time-inconsistent (winners renege after).

**Layer 3 — Mishra:** uprooting → resentment → rage looking for political form. 19th-century echoes.

**Demand vs supply of populism (Rodrik):**
- **Demand:** globalization shocks → grievance (universal).
- **Supply:** type of shock determines cleavage activated.
  - Trade/finance shocks → class → left populism (LatAm: Chávez, Morales).
  - Immigration shocks → ethno-cultural → right populism (Europe: Le Pen, Orbán).
  - US 2010s = both shocks → Sanders + Trump on different cleavages of the same crisis.

==**Trap to avoid:** "Globalization explains right-wing populism" is *lazy*. Globalization explains the *demand*, not the *form*.==

**Populism vs nationalism:** populism = repertoire (portable style); nationalism = structure (long-running political community). East Asia (Japan, Korea, Taiwan) = strong nationalisms, no durable populist parties — proof they're different.

**Line to memorise:**
> *"Self-seeking individuals with very different pasts now find themselves herded together into a global marketplace where humiliating new hierarchies emerge."* — Mishra (2016)

---

### Week 12 — Anthropocene and Global Futures

**Frame:** Is the IR toolkit equipped for planetary problems? Mitchell: the discipline keeps extinction in the footnotes.

**Section A — Mitchell's central claim:**
- Extinction is now a stake; IR claims to be a "theory of survival" yet has no framework for it.
- ==**Structural mismatch:** IR's unit is the state (bounded, territorial, short-term); planetary processes (climate, biodiversity) cross borders and operate on geological timescales.==
- **The ontological gap:** standard response is *better management* (treaties, targets) — but the field cannot yet *think* the loss of life forms.
- **Anthropocentrism = blind spot:** IR centers "humanity" as the only subject; nonhumans = backdrop; taboo is *structural, not evidential*.
- **Mitchell proposes:** post-humanism, planetary thinking, cosmopolitics, ethics of gratitude beyond survival.

**Section B — four future challenges:**
1. **Technology / AI:** capability outruns governance; dual-use; autonomy in weapons; risk unevenly distributed.
2. **Space frontier:** great-power competition extended beyond Earth; 1960s treaties for a different industry; SpaceX-era private actors.
3. **Ecology / planetary boundaries:** nine boundaries (climate, biodiversity, biogeochemical, freshwater, land-use, ocean acid, aerosols, ozone, novel entities); several already exceeded; tipping points are nonlinear; "resilience" can shift costs onto the vulnerable.
4. **Migration:** climate displacement at unprecedented scale; refugee law for a different crisis; internal > cross-border; border politics shaped by *anticipated* flows. **Fertility decline / global relationship recession** (Burn-Murdoch 2025) — TFR<2.1 reshapes labor, pensions, migration, geopolitical balance.

**Course through-lines (closing):**
1. Constructivism + Global IR — sister critiques of mainstream.
2. Nationalism (structure) vs populism (repertoire) — analytically separable, empirically fused.
3. Rodrik's trilemma — runs through Weeks 8, 11, 12.

**Meta-lesson:** IR is a contested toolkit. Every tool answers a different question. Post-1989 settlement frays at multiple seams.

**Line to remember (close):**
> *"We are more connected than ever — but are we more together?"* — Denney close

